...
from explore1_utils import find_latest_run_artifact, flatten_risk_summary, relpath, topic_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect Explore1 universe, model, backtest and baseline reports.")
    parser.add_argument("--universe", default="Explore1/data/universe/mcap500_mainboard_20251231.csv")
    parser.add_argument("--audit", default="Explore1/outputs/reports/mcap500_universe_audit.csv")
    parser.add_argument("--data-quality", default="Explore1/outputs/reports/data_quality_report.csv")
    parser.add_argument("--grid-summary", default="Explore1/outputs/reports/alpha158_topk_grid_summary.csv")
    parser.add_argument("--equal-weight-summary", default="Explore1/outputs/reports/equal_weight_monthly_summary.csv")
    parser.add_argument("--mlruns-dir", default="mlruns")
    parser.add_argument("--new-experiment", default="alpha158_lightgbm_mcap500")
    parser.add_argument("--old-experiment", default="alpha158_lightgbm_selected")
    parser.add_argument("--output-md", default="Explore1/outputs/reports/explore1_report.md")
    parser.add_argument("--output-csv", default="Explore1/outputs/reports/explore1_summary.csv")
...
def main() -> int:
    ...
    if not grid_enriched.empty:
        for _, row in grid_enriched.iterrows():
            record = {"kind": "alpha158_topk_grid"}
...
- Qlib provider：`Explore1/data/qlib/cn_data`

{md_table(data_rows, ["metric", "value"])}

### 历史最短的 10 只股票

这些股票多为近年上市，Alpha158 的早期训练期样本不足，Qlib 会自然按可用日期参与训练和预测。
...
- `n_drop` 越大，换手和成本明显增加。例如 `topk=30,n_drop=10` 平均换手约 `64.24%`，平均日成本约 `0.0641%`，扣成本后收益低于 `topk=30,n_drop=3`。
- `topk=100` 的回撤普遍更低，说明更分散的持仓能降低波动；但收益弹性弱于 `topk=50,n_drop=3`。
...
- 月度等权基准扣成本超额年化 `{fmt_pct(ew_row.get("excess_return_with_cost.annualized_return"))}`，明显低于最佳 Alpha158 网格的 `{fmt_pct(best_grid_row.get("excess_return_with_cost.annualized_return"))}`。
...
- Workflow 已完整跑通：股票池构建、日线抓取、Qlib dump、Alpha158/LGBM 训练、9 组 TopK 网格、月度等权基准、汇总报告都已生成。
...
- 当前最好结果来自 `topk=50,n_drop=3`，它在收益和成本之间比较平衡；`topk=100,n_drop=5` 的 IR 和回撤更稳，适合作为更保守的候选。
- 不要把这轮结果解读为“Alpha158 已经有效”。Rank IC 仍很低，且股票池使用未来日期静态筛选，收益可能混入了大盘风格、幸存者和未来函数。
