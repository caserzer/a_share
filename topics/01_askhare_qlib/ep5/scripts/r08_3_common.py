...
DEFAULT_CONFIG = EP5_DIR / "configs" / "r08_3_daily_observed_volume_rank_families_h3_h5_h10_transferability_diagnostic_v0.yaml"

REQUIREMENT_ID = "ep5_r08_3_daily_observed_volume_rank_families_h3_h5_h10_transferability_diagnostic_v0"
PLAN_ID = "ep5_e08_3_daily_observed_volume_rank_families_h3_h5_h10_transferability_diagnostic_v0"
PRIMARY_FAMILIES = ["volume_surge_money_flow", "volume_price_correlation", "rank_ts_rank_structure"]
R07_REFERENCE_FACTOR_COUNTS = {
    "volume_surge_money_flow": 15,
    "volume_price_correlation": 3,
    "rank_ts_rank_structure": 1,
}
HORIZONS = [3, 5, 10]
PRIMARY_HORIZON = 3
...
def load_config(path: str | Path = DEFAULT_CONFIG) -> tuple[dict[str, Any], R083Paths]:
    import yaml
...
def artifact_hashes(paths: R083Paths) -> list[dict[str, Any]]:
    rows = []
...
def build_daily_signal_panel(config: dict[str, Any], paths: R083Paths, feature: pd.DataFrame) -> pd.DataFrame:
    ...
    out.to_parquet(paths.cache_dir / "r08_3_daily_signal_panel.parquet", index=False)
    audit_rows = []
    for split in SPLITS:
    ...
    write_csv(pd.DataFrame(audit_rows), paths.audit_dir / "r08_3_daily_signal_panel_audit.csv")
    return out
...
def build_execution_and_labels(
    ...
    execution.to_parquet(paths.cache_dir / "r08_3_execution_label_panel.parquet", index=False)
    label.to_parquet(paths.cache_dir / "r08_3_label_panel.parquet", index=False)
    write_csv(pd.DataFrame(availability_rows), paths.audit_dir / "r08_3_data_availability_by_horizon_audit.csv")
...
def write_label_audit(config: dict[str, Any], paths: R083Paths, label: pd.DataFrame) -> None:
    rows = []
    for horizon in HORIZONS:
        h = f"H{horizon}"
    ...
    write_csv(pd.DataFrame(rows), paths.audit_dir / "r08_3_label_asof_audit.csv")
...
def build_factor_state_inputs(
    ...
    for fid in factor_ids:
        ...
        try:
            ...
            print(f"R08.3 factor skipped: {fid}: {exc}", flush=True)
            continue
        ...
        print(f"R08.3 factor normalized: {fid}", flush=True)
    ...
    np.save(paths.cache_dir / "r08_3_raw_target_factor_matrix.npy", raw_matrix)
    np.save(paths.cache_dir / "r08_3_within_stock_percentile_matrix.npy", percentile_matrix)
    write_json({"factor_ids": available_fids}, paths.cache_dir / "r08_3_factor_matrix_columns.json")
...
def build_scope_audits(
    ...
    write_csv(
        ...
        paths.audit_dir / "r08_3_input_data_audit.csv",
    )
    write_csv(
        pd.DataFrame(
            [
        ...
        paths.audit_dir / "r08_3_scope_audit.csv",
    )
    fold_rows = []
    ...
    write_csv(pd.DataFrame(fold_rows), paths.audit_dir / "r08_3_fold_assignment_audit.csv")
...
def build_normalization_audit(
    ...
    write_csv(pd.DataFrame(rows), paths.audit_dir / "r08_3_within_stock_normalization_audit.csv")
...
def direction_by_fold(
    ...
    write_csv(direction_df, paths.audit_dir / "r08_3_factor_direction_by_fold_audit.csv")
    write_csv(nonconst_df, paths.audit_dir / "r08_3_factor_nonconstant_observation_audit.csv")
...
def build_oof_events(
    ...
    write_csv(scope_df, paths.audit_dir / "r08_3_family_scope_by_fold_audit.csv")
    write_csv(bucket_df, paths.audit_dir / "r08_3_state_bucket_by_fold_audit.csv")
    write_csv(validity_df, paths.audit_dir / "r08_3_horizon_specific_instrument_validity_audit.csv")
    if not events.empty:
        events.to_parquet(paths.cache_dir / "r08_3_oof_event_panel.parquet", index=False)
...
def build_metrics(config: dict[str, Any], paths: R083Paths, events: pd.DataFrame) -> dict[str, pd.DataFrame]:
    c = config["frozen_formula_constants"]
    full_rows = []
    anchor_rows = []
    fold_rows = []
...
def write_metric_artifacts(paths: R083Paths, frames: dict[str, pd.DataFrame]) -> None:
    write_csv(frames["full"].loc[frames["full"]["horizon"].eq("H3")] if not frames["full"].empty else frames["full"], paths.metrics_dir / "r08_3_h3_full_daily_oof_spread.csv")
    write_csv(frames["anchor"].loc[frames["anchor"]["horizon"].eq("H3")] if not frames["anchor"].empty else frames["anchor"], paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread.csv")
    write_csv(frames["anchor"].loc[(frames["anchor"]["horizon"].eq("H3")) & (frames["anchor"]["split"].eq("train_oof_unseen"))] if not frames["anchor"].empty else frames["anchor"], paths.metrics_dir / "r08_3_h3_train_baseline_summary.csv")
    write_csv(frames["fold"].loc[frames["fold"]["horizon"].eq("H3")] if not frames["fold"].empty else frames["fold"], paths.metrics_dir / "r08_3_h3_fold_unseen_state_spread.csv")
    write_csv(frames["dispersion"].loc[frames["dispersion"]["horizon"].eq("H3")] if not frames["dispersion"].empty else frames["dispersion"], paths.metrics_dir / "r08_3_h3_fold_dispersion_summary.csv")
    write_csv(frames["instrument"].loc[frames["instrument"]["horizon"].eq("H3")] if not frames["instrument"].empty else frames["instrument"], paths.metrics_dir / "r08_3_h3_instrument_transfer_summary.csv")
    write_csv(frames["time"].loc[frames["time"]["horizon"].eq("H3")] if not frames["time"].empty else frames["time"], paths.metrics_dir / "r08_3_h3_time_transfer_summary.csv")
    write_csv(frames["year"].loc[frames["year"]["horizon"].eq("H3")] if not frames["year"].empty else frames["year"], paths.metrics_dir / "r08_3_h3_year_availability_and_positive_count.csv")
    write_csv(frames["decile"].loc[frames["decile"]["horizon"].eq("H3")] if not frames["decile"].empty else frames["decile"], paths.metrics_dir / "r08_3_h3_decile_monotonicity_by_anchor.csv")
    write_csv(frames["concentration"].loc[frames["concentration"]["horizon"].eq("H3")] if not frames["concentration"].empty else frames["concentration"], paths.metrics_dir / "r08_3_h3_concentration_summary.csv")
    write_csv(frames["full"].loc[frames["full"]["horizon"].eq("H5")] if not frames["full"].empty else frames["full"], paths.metrics_dir / "r08_3_h5_diagnostic_oof_spread.csv")
    write_csv(frames["full"].loc[frames["full"]["horizon"].eq("H10")] if not frames["full"].empty else frames["full"], paths.metrics_dir / "r08_3_h10_diagnostic_oof_spread.csv")
    write_csv(frames["horizon_shape"], paths.metrics_dir / "r08_3_horizon_shape_summary.csv")
    write_csv(frames["overlap"], paths.metrics_dir / "r08_3_overlap_adjusted_confidence_summary.csv")
    write_csv(frames["sample"], paths.audit_dir / "r08_3_fold_sample_audit.csv")
    write_csv(frames["concentration"], paths.audit_dir / "r08_3_concentration_audit.csv")
...
def build_gate_inputs(
    ...
    write_csv(gate, paths.decision_dir / "r08_3_gate_inputs.csv")
    write_csv(diagnostic_inputs, paths.decision_dir / "r08_3_horizon_diagnostic_inputs.csv")
...
def build_final_decision(paths: R083Paths, gate: pd.DataFrame, diagnostic: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rules = [
        ("rule_01", "data / execution / scope / as-of / fold contract violation", contract_violation, "r08_3_blocked_data_or_execution_contract"),
        ("rule_02", "primary vwap family cannot form fold-specific state score", score_missing, "r08_3_blocked_overlap_controlled_sample_insufficient"),
        ("rule_03", "aggregate_oof_sample_status = fail or H3 anchor sample gate fails", sample_fail, "r08_3_blocked_overlap_controlled_sample_insufficient"),
        ("rule_04", "all non-fold H3 support gates pass and fold stability fails", time_pass and inst_pass and anchor_pass and mono_pass and conc_pass and rob_nd_pass and not fold_pass, "r08_3_daily_vwap_h3_fold_fragile_candidate"),
        ("rule_05", "time transfer passes, instrument transfer fails, other H3 cleanliness gates pass", time_pass and not inst_pass and fold_pass and anchor_pass and mono_pass and conc_pass and rob_nd_pass, "r08_3_daily_vwap_h3_time_transfer_only"),
        ("rule_06", "all H3 support gates pass", support_all, "r08_3_daily_vwap_h3_transferability_diagnostic_supported"),
        ("rule_07", "H3 sample passes and H3 support fails, while H5/H10 diagnostic horizon passes", sample_pass and not support_all and hdiag, "r08_3_horizon_mismatch_diagnostic_only"),
        ("rule_08", "otherwise", True, "r08_3_no_daily_vwap_h3_transferability_support"),
    ]
    selected_decision = ""
    replay_rows = []
    selected_seen = False
        ...
        [
            {
                ...
                if selected_decision == "r08_3_daily_vwap_h3_transferability_diagnostic_supported"
                else "",
                "primary_family": PRIMARY_FAMILY,
                "primary_horizon": "H3",
                "diagnostic_horizons": "H5;H10",
    ...
    write_csv(replay, paths.decision_dir / "r08_3_final_decision_replay.csv")
    write_csv(final, paths.decision_dir / "r08_3_final_decision.csv")
    return replay, final


def write_report(paths: R083Paths, gate: pd.DataFrame, diagnostic: pd.DataFrame, final: pd.DataFrame) -> None:
    decision = final.iloc[0]["final_decision"] if len(final) else ""
    h3_anchor = pd.read_csv(paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread.csv")
    h3_fold = pd.read_csv(paths.metrics_dir / "r08_3_h3_fold_dispersion_summary.csv")
    hshape = pd.read_csv(paths.metrics_dir / "r08_3_horizon_shape_summary.csv")
    primary = gate.iloc[0] if len(gate) else None
    lines = [
        "# R08.3 Daily-Observed VWAP Deviation H3/H5/H10 Transferability Diagnostic Report",
        "",
        "## 1. 结论",
        "",
        f"`final_decision = {decision}`",
        "",
        "`authorized_strategy_requirement = false`。R08.3 是 daily-observed diagnostic-only audit，没有策略、组合、top-N、top20% 或 production signal。",
        "",
        "R08.3 只改变 signal observation frequency：从 weekly close-observed 改为 daily close-observed。Primary family 固定为 `vwap_deviation`，primary horizon 固定为 H3；H5/H10 仅用于 horizon-shape diagnostic。",
        "",
    ...
    qas = [
        "1. R08.3 是否保持 diagnostic-only，且没有构造任何策略？是。",
    ...
    lines.extend(qas)
    (paths.reports_dir / "r08_3_final_report.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
...
def data_availability_by_horizon(
    ...
    c = config["frozen_formula_constants"]
    provider_end = pd.Timestamp(config["data_sources"].get("provider_load_end_date", config["split"]["robustness_end"])).normalize()
    signal_dates = pd.to_datetime(candidates["signal_date"]).dt.normalize()
    exit_dates = pd.to_datetime(exit_date)
    complete_s = pd.Series(complete)
    rows = []
    ...
    for split_name in SPLITS:
        declared = pd.Timestamp(config["split"][f"{split_name}_end"]).normalize()
        split_mask = candidates["split"].eq(split_name)
        complete_split = complete_s & split_mask.reset_index(drop=True)
        complete_signal = signal_dates[complete_split].reset_index(drop=True)
        complete_exit = exit_dates[complete_split]
        last_complete_signal = complete_signal.max() if len(complete_signal) else pd.NaT
        last_available = min(provider_end, complete_exit.max().normalize() if len(complete_exit.dropna()) else provider_end)
        actual_end = min(declared, last_available, last_complete_signal) if not pd.isna(last_complete_signal) else pd.NaT
        evaluable = candidates.loc[
            complete
            & split_mask.to_numpy()
            & (pd.to_datetime(candidates["signal_date"]) <= actual_end)
        ].copy()
        year_counts = (
            evaluable.groupby(pd.to_datetime(evaluable["signal_date"]).dt.year)["signal_date"].nunique()
            if len(evaluable)
        ...
        rows.append(
            {
                "horizon": horizon,
                "split": split_name,
                "declared_split_end_date": declared,
                "last_available_trading_date": last_available,
                "last_label_complete_signal_date": last_complete_signal,
                "split_window_actual_end_date": actual_end,
                "split_end_date_data_available": bool(not pd.isna(actual_end) and actual_end >= declared),
                "split_window_truncated_by_data_availability": bool(pd.isna(actual_end) or actual_end < declared),
                "split_actual_evaluable_year_count": len(evaluable_years),
                "split_actual_evaluable_years": ";".join(str(x) for x in evaluable_years),
                "split_actual_signal_date_count": int(evaluable["signal_date"].nunique()),
                "evaluable_year_signal_date_floor": threshold,
            }
        )
    return rows
...
def family_factor_scope(inputs: r08.R06Inputs) -> dict[str, list[str]]:
    included = set(inputs.factor_ids)
    scope: dict[str, list[str]] = {}
    for family in PRIMARY_FAMILIES:
        scope[family] = sorted(
            inputs.family_map.loc[
                inputs.family_map["primary_family"].eq(family)
                & inputs.family_map["factor_id"].isin(included),
                "factor_id",
            ].astype(str)
        )
...
def scope_factor_ids(inputs: r08.R06Inputs) -> list[str]:
    ids: list[str] = []
    for fids in family_factor_scope(inputs).values():
        ids.extend(fids)
    return sorted(set(ids))


def factor_to_family_map(scope: dict[str, list[str]]) -> dict[str, str]:
    return {fid: family for family, fids in scope.items() for fid in fids}


def retained_floor(config: dict[str, Any], family: str, in_scope_count: int) -> int:
    c = config["frozen_formula_constants"]
    count_floor = int(c["retained_factor_count_floor"][family])
    share_floor = math.ceil(float(c["retained_factor_min_family_share"][family]) * in_scope_count)
    return max(count_floor, share_floor)


def build_scope_audits_multi(
    paths: R083Paths,
    inputs: r08.R06Inputs,
    candidates: pd.DataFrame,
    scope: dict[str, list[str]],
            ...
            [
                {
                    ...
                    "min_signal_date": candidates["signal_date"].min(),
                    "max_signal_date": candidates["signal_date"].max(),
                    "primary_families": ";".join(PRIMARY_FAMILIES),
                    "primary_horizon": "H3",
                    "diagnostic_horizons": "H5;H10",
    ...
    for family, fids in scope.items():
        map_count = int(inputs.family_map["primary_family"].eq(family).sum())
        executable_count = len(fids)
        reference_count = R07_REFERENCE_FACTOR_COUNTS[family]
        changed = executable_count != reference_count
        explained = not changed
        scope_rows.append(
            {
                "family": family,
                "role": "co_primary_diagnostic",
                "r06_family_map_primary_count": map_count,
                "r06_executable_in_scope_factor_count": executable_count,
                "r07_reference_executable_factor_count": reference_count,
                "family_scope_reference_count_changed": changed,
                "family_scope_reference_count_change_explained": explained,
                "unexplained_family_scope_reference_count_changed": changed and not explained,
                "in_scope_factor_ids": ";".join(fids),
                "available_factor_ids": ";".join(fid for fid in fids if fid in available),
                "primary_decision_eligible": True,
                "single_factor_family_caveat": family == "rank_ts_rank_structure",
            }
        )
        for fid in fids:
            factor_rows.append(
                {
                    "family": family,
                    "factor_id": fid,
                    "in_r06_factor_matrix_columns": fid in inputs.factor_ids,
                    "available_in_runtime_factor_matrix": fid in available,
                    "r06_family_map_primary_family": family,
                }
            )
    write_csv(pd.DataFrame(scope_rows), paths.audit_dir / "r08_3_scope_audit.csv")
    write_csv(pd.DataFrame(factor_rows), paths.audit_dir / "r08_3_family_factor_scope_audit.csv")
    fold_rows = []
    inst = candidates[["instrument_id", "canonical_instrument_id", "fold_hash_value", "instrument_fold_id"]].drop_duplicates()
    for rec in inst.itertuples(index=False):
        row = {
        ...
        for split in SPLITS:
            row[f"{split}_signal_count"] = int(
                candidates.loc[
                    candidates["instrument_id"].eq(rec.instrument_id)
...
def build_normalization_audit_multi(
    paths: R083Paths,
    candidates: pd.DataFrame,
    available_fids: list[str],
    percentile: np.ndarray,
...
def direction_by_family_fold(
    ...
    c = config["frozen_formula_constants"]
    fid_to_col = {fid: i for i, fid in enumerate(available_fids)}
    train = candidates.merge(label[["candidate_row_id", "label_self_relative_H3"]], on="candidate_row_id", how="left")
    rows = []
    nonconst_rows = []
    directions: dict[str, dict[int, dict[str, float]]] = {family: {} for family in PRIMARY_FAMILIES}
    min_count = int(c["min_direction_signal_count_for_instrument_factor"])
    min_anchor = int(c["min_direction_anchor_signal_count_for_instrument_factor"])
    for family in PRIMARY_FAMILIES:
        for fold_id in FOLD_IDS:
            ...
            for fid in scope[family]:
                ...
                ok = sample_ok and nonconst_ok and anchor_stable
                if ok:
                    directions[family][fold_id][fid] = float(direction)
                rows.append(
                    {
                        "family": family,
                        "fold_id": fold_id,
    ...
    direction_df = pd.DataFrame(rows)
    nonconst_df = pd.DataFrame(nonconst_rows)
    write_csv(direction_df, paths.audit_dir / "r08_3_factor_direction_by_family_fold_audit.csv")
    write_csv(nonconst_df, paths.audit_dir / "r08_3_factor_nonconstant_observation_audit.csv")
    return direction_df, nonconst_df, directions


def factor_set_comparability(
    family: str,
    retained_by_fold: dict[int, dict[str, float]],
    min_jaccard: float,
) -> tuple[bool, bool, float, bool]:
    sets = [set(retained_by_fold.get(k, {}).keys()) for k in FOLD_IDS]
    identical = all(s == sets[0] for s in sets)
    pairwise = []
    for i in range(len(sets)):
        for j in range(i + 1, len(sets)):
            union = sets[i] | sets[j]
            pairwise.append(len(sets[i] & sets[j]) / len(union) if union else 0.0)
    min_pairwise = min(pairwise) if pairwise else 0.0
    if family in {"rank_ts_rank_structure", "volume_price_correlation"}:
        return identical, identical, min_pairwise, True
    comparable = identical or min_pairwise >= min_jaccard
    return comparable, identical, min_pairwise, not identical


def count_bucket_edges(values: pd.Series, tail_share: float) -> tuple[float, float]:
    vals = values.replace([np.inf, -np.inf], np.nan).dropna().sort_values(kind="mergesort")
    if vals.empty:
        return np.nan, np.nan
    tail_count = int(math.ceil(tail_share * len(vals)))
    q20 = float(vals.iloc[max(0, min(len(vals) - 1, tail_count - 1))])
    desc = vals.iloc[::-1].reset_index(drop=True)
    q80 = float(desc.iloc[max(0, min(len(desc) - 1, tail_count - 1))])
    return q20, q80


def build_oof_events_multi(
    config: dict[str, Any],
    paths: R083Paths,
    candidates: pd.DataFrame,
    label: pd.DataFrame,
    ...
    available_fids: list[str],
    percentile: np.ndarray,
    directions: dict[str, dict[int, dict[str, float]]],
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    c = config["frozen_formula_constants"]
    scope_rows = []
    bucket_rows = []
    ...
    label_cols = ["candidate_row_id"] + [f"label_self_relative_H{h}" for h in HORIZONS] + [f"H{h}_complete_flag" for h in HORIZONS]
    base = candidates.merge(label[label_cols], on="candidate_row_id", how="left")
    min_jaccard = float(c["retained_factor_set_min_pairwise_jaccard"])
    family_comparable = {}
    for family in PRIMARY_FAMILIES:
        comparable, identical, min_pairwise, changed_explained = factor_set_comparability(family, directions[family], min_jaccard)
        family_comparable[family] = comparable
        for fold_id in FOLD_IDS:
            retained = directions[family].get(fold_id, {})
            in_scope_count = len(scope[family])
            floor = retained_floor(config, family, in_scope_count)
            scope_pass = len(retained) >= floor and comparable
            scope_rows.append(
                {
                    "family": family,
                    "fold_id": fold_id,
                    "role": "co_primary_diagnostic",
                    "in_scope_factor_count": in_scope_count,
                    "retained_factor_floor": floor,
                    "retained_factor_count": len(retained),
                    "retained_factor_ids": ";".join(retained),
                    "dropped_factor_ids": ";".join(fid for fid in scope[family] if fid not in retained),
                    "retained_factor_set_identical": identical,
                    "retained_factor_set_min_pairwise_jaccard": min_pairwise,
                    "retained_factor_set_change_explained": changed_explained,
                    "retained_factor_set_comparability_pass": comparable,
                    "direction_insufficient_factors_dropped": True,
                    "family_scope_pass": scope_pass,
                    "single_factor_family_caveat": family == "rank_ts_rank_structure",
                    "primary_decision_eligible": True,
            ...
            train_seen = tmp["split"].eq("train") & tmp["instrument_fold_id"].ne(fold_id)
            train_vals = tmp.loc[train_seen, "family_state_score"].replace([np.inf, -np.inf], np.nan).dropna()
            q20, q80 = count_bucket_edges(train_vals, float(c["bucket_tail_count_share"]))
            decile_edges = train_vals.quantile([i / 10 for i in range(1, 10)]).astype(float).tolist() if len(train_vals) else [np.nan] * 9
            state, decile = state_from_edges(tmp["family_state_score"], q20, q80, decile_edges)
            tmp["state"] = state
            tmp["state_decile"] = decile
            bottom_share = safe_share(int((state == "bottom_quintile_state").sum()), int(np.isfinite(score).sum()))
            top_share = safe_share(int((state == "top_quintile_state").sum()), int(np.isfinite(score).sum()))
            tie_q20 = safe_share(int((tmp["family_state_score"] == q20).sum()), int(np.isfinite(score).sum())) if finite(q20) else np.nan
            tie_q80 = safe_share(int((tmp["family_state_score"] == q80).sum()), int(np.isfinite(score).sum())) if finite(q80) else np.nan
            bucket_valid = (
                finite(q20)
                and finite(q80)
                and q20 < q80
                and bottom_share >= float(c["bucket_actual_tail_share_min"])
                and bottom_share <= float(c["bucket_actual_tail_share_max"])
                and top_share >= float(c["bucket_actual_tail_share_min"])
                and top_share <= float(c["bucket_actual_tail_share_max"])
            )
            bucket_rows.append(
                {
                    "family": family,
                    ...
                    "bucket_edge_source_instrument_scope": f"seen_folds_not_{fold_id}",
                    "bucket_frequency": "daily",
                    "train_seen_event_count": int(len(train_vals)),
                    "tail_count": int(math.ceil(float(c["bucket_tail_count_share"]) * len(train_vals))) if len(train_vals) else 0,
                    "train_seen_q20": q20,
                    "train_seen_q80": q80,
                    "train_seen_bottom_state_share": bottom_share,
                    "train_seen_top_state_share": top_share,
                    "bucket_boundary_tie_share_q20": tie_q20,
                    "bucket_boundary_tie_share_q80": tie_q80,
                    "bucket_edge_valid": bucket_valid,
                    "decile_edges_train_seen": json.dumps(decile_edges),
                    "frozen_before_validation_read": True,
                    "bucket_method": "fold_train_seen_daily_count_based_extreme_tail_20_60_20",
                }
            )
            if not (scope_pass and bucket_valid):
                continue
            unseen = tmp.loc[tmp["instrument_fold_id"].eq(fold_id)].copy()
            for horizon in HORIZONS:
                h = f"H{horizon}"
                ...
                hdf["label_complete_flag"] = hdf[f"{h}_complete_flag"]
                hdf = hdf.loc[
                    hdf["label_complete_flag"].map(bool_value)
                    & hdf["label_self_relative"].notna()
                    & hdf["family_state_score"].notna()
                ].copy()
                if hdf.empty:
                    continue
                hdf["oof_split"] = hdf["split"].map(SPLIT_TO_OOF)
                ...
                class_frames = []
                for oof_split, g in hdf.groupby("oof_split", sort=False):
                    full_min = int(c["train_full_instrument_signal_count_min"] if oof_split == "train_oof_unseen" else c["validation_robustness_full_instrument_signal_count_min"])
                    partial_min = int(c["partial_instrument_signal_count_min"])
                    counts = g.groupby("instrument_id")["signal_date"].nunique()
                    htmp = g.copy()
                    htmp["split_signal_count_for_instrument"] = htmp["instrument_id"].map(counts).astype(int)
                    htmp["instrument_sample_class"] = np.select(
                        [
                            htmp["split_signal_count_for_instrument"] >= full_min,
                            htmp["split_signal_count_for_instrument"] >= partial_min,
                        ],
                        ["full_valid_instrument", "partial_instrument_event_only"],
                        default="excluded_thin_instrument",
                    )
                    htmp["full_instrument_signal_count_floor"] = full_min
                    htmp["partial_instrument_signal_count_floor"] = partial_min
                    validity_rows.append(
                        {
                            "family": family,
                            "horizon": h,
                    ...
                    class_frames.append(htmp.loc[htmp["instrument_sample_class"].ne("excluded_thin_instrument")])
                if class_frames:
                    event_frames.append(pd.concat(class_frames, ignore_index=True))
    scope_df = pd.DataFrame(scope_rows)
    bucket_df = pd.DataFrame(bucket_rows)
    ...
    events = pd.concat(event_frames, ignore_index=True) if event_frames else pd.DataFrame()
    write_csv(scope_df, paths.audit_dir / "r08_3_family_scope_by_fold_audit.csv")
    write_csv(bucket_df, paths.audit_dir / "r08_3_state_bucket_by_family_fold_audit.csv")
    write_csv(validity_df, paths.audit_dir / "r08_3_horizon_specific_instrument_validity_audit.csv")
    if not events.empty:
        events.to_parquet(paths.cache_dir / "r08_3_oof_event_panel.parquet", index=False)
    return events, scope_df, bucket_df, validity_df
...
def contribution_summary(df: pd.DataFrame, label_col: str, family: str, split: str, fold_id: Any, anchor_offset: Any = "all") -> dict[str, Any]:
    state_df = df.loc[df["state"].isin(["top_quintile_state", "bottom_quintile_state"])].copy()
    state_df[label_col] = state_df[label_col].replace([np.inf, -np.inf], np.nan)
    state_df = state_df.dropna(subset=[label_col])
    signed_frames = []
    for _, g in state_df.groupby("signal_date", sort=False):
        top = g["state"].eq("top_quintile_state")
        bottom = g["state"].eq("bottom_quintile_state")
        if int(top.sum()) == 0 or int(bottom.sum()) == 0:
            continue
        tmp = g.copy()
        tmp["signed_event_contribution"] = 0.0
        tmp.loc[top, "signed_event_contribution"] = tmp.loc[top, label_col] / int(top.sum())
        tmp.loc[bottom, "signed_event_contribution"] = -tmp.loc[bottom, label_col] / int(bottom.sum())
        signed_frames.append(tmp)
    signed = pd.concat(signed_frames, ignore_index=True) if signed_frames else pd.DataFrame()
    if signed.empty:
        denom = 0.0
        return {
            "family": family,
            ...
            "anchor_offset": anchor_offset,
            "contribution_denominator": denom,
            "contribution_denominator_zero": True,
            "top1_instrument_id": "",
            "top1_instrument_event_count": 0,
            "top1_instrument_contribution_share": np.nan,
            "top5_instrument_contribution_share": np.nan,
            "top1_industry": "",
            "top1_industry_contribution_share": np.nan,
            "fold_contribution_share_of_total_abs_contribution": np.nan,
        }
    inst_signed = signed.groupby("instrument_id")["signed_event_contribution"].sum()
    inst_abs = inst_signed.abs().sort_values(ascending=False)
    denom = float(inst_abs.sum())
    zero = denom <= 1e-12
    top1_id = str(inst_abs.index[0]) if len(inst_abs) else ""
    top1_share = float(inst_abs.iloc[0] / denom) if not zero and len(inst_abs) else np.nan
    top5_share = float(inst_abs.head(5).sum() / denom) if not zero and len(inst_abs) else np.nan
    top1_count = int(signed["instrument_id"].eq(top1_id).sum()) if top1_id else 0
    inst_industry = signed.drop_duplicates("instrument_id").set_index("instrument_id")["industry_id"].to_dict()
    industry_abs: dict[Any, float] = {}
    for instrument, value in inst_abs.items():
        industry = inst_industry.get(instrument, "")
        industry_abs[industry] = industry_abs.get(industry, 0.0) + float(value)
    if industry_abs and not zero:
        top_industry, top_industry_value = max(industry_abs.items(), key=lambda kv: kv[1])
        top_industry_share = float(top_industry_value / denom)
    else:
        top_industry, top_industry_share = "", np.nan
    fold_abs = signed.groupby("instrument_fold_id")["signed_event_contribution"].sum().abs()
    fold_share = float(fold_abs.max() / denom) if not zero and len(fold_abs) else np.nan
    return {
        "family": family,
        "split": split,
        "fold_id": fold_id,
        ...
        "top1_instrument_contribution_share": top1_share,
        "top5_instrument_contribution_share": top5_share,
        "top1_industry": str(top_industry),
        "top1_industry_contribution_share": top_industry_share,
        "fold_contribution_share_of_total_abs_contribution": fold_share,
    }


def build_metrics_multi(config: dict[str, Any], paths: R083Paths, events: pd.DataFrame) -> dict[str, pd.DataFrame]:
    c = config["frozen_formula_constants"]
    full_rows: list[dict[str, Any]] = []
    anchor_rows: list[dict[str, Any]] = []
    fold_rows: list[dict[str, Any]] = []
    fold_disp_rows: list[dict[str, Any]] = []
    ...
    horizon_shape_rows: list[dict[str, Any]] = []
    family_comparison_rows: list[dict[str, Any]] = []
    if events.empty:
        frames = {name: pd.DataFrame() for name in ["full", "anchor", "fold", "dispersion", "instrument", "time", "year", "decile", "sample", "concentration", "overlap", "horizon_shape", "family_comparison"]}
        write_metric_artifacts_multi(paths, frames)
        return frames
    for family in PRIMARY_FAMILIES:
        family_events = events.loc[events["family"].eq(family)].copy()
        for horizon in HORIZONS:
            h = f"H{horizon}"
            h_events = family_events.loc[family_events["horizon"].eq(h)].copy()
            for split in OOF_SPLITS:
                split_events = h_events.loc[h_events["oof_split"].eq(split)].copy()
                if split_events.empty:
                    continue
                full_metric, _, full_inst, _, full_conc = subset_metrics(split_events, c, "label_self_relative", family, split, "aggregate")
                row = prefixed_metric_row(h, split, "aggregate", full_metric, prefix=f"{h.lower()}_full_daily")
                row["family"] = family
                ...
                for anchor in range(horizon):
                    a_events = split_events.loc[(split_events["daily_trading_calendar_index"].astype(int) % horizon) == anchor].copy()
                    metric, _, _, _, conc = subset_metrics(a_events, c, "label_self_relative", family, split, "aggregate", anchor)
                    anchor_metrics.append(metric)
                    anchor_mean_values.append(metric["mean_spread"])
                    conc_rows.append(dict(conc, horizon=h, scope="aggregate_anchor"))
                    overlap_rows.append(
                        {
                            "family": family,
                ...
                anchor_mean_s = pd.Series(anchor_mean_values).replace([np.inf, -np.inf], np.nan).dropna()
                combined_metric, combined_spreads, _, combined_decile, combined_conc = subset_metrics(split_events, c, "label_self_relative", family, split, "aggregate")
                anchor_controlled_mean = safe_mean(anchor_mean_s)
                anchor_controlled_median = safe_median(anchor_mean_s)
                positive_anchor_count = int((anchor_mean_s > 0).sum())
                min_anchor = float(anchor_mean_s.min()) if len(anchor_mean_s) else np.nan
                max_anchor_share = float(anchor_mean_s.abs().max() / anchor_mean_s.abs().sum()) if len(anchor_mean_s) and anchor_mean_s.abs().sum() > 0 else np.nan
                anchor_row = {
                    "family": family,
                    "horizon": h,
                    "split": split,
                    ...
                    "anchor_positive_offset_count": positive_anchor_count,
                    "anchor_offset_spread_min": min_anchor,
                    "anchor_offset_spread_median": safe_median(anchor_mean_s),
                    "anchor_controlled_positive_instrument_share": combined_metric["positive_instrument_share"],
                    "anchor_controlled_valid_instrument_count": combined_metric["valid_instrument_count"],
                    "anchor_controlled_full_valid_instrument_count": combined_metric["full_valid_instrument_count"],
                    "anchor_controlled_valid_signal_date_count": combined_metric["valid_signal_date_count"],
                ...
                for fold_id in FOLD_IDS:
                    ...
                    if f_events.empty:
                        ...
                        fold_anchor_min = np.nan
                        fold_anchor_positive = 0
                        conc = contribution_summary(f_events, "label_self_relative", family, split, fold_id)
                    else:
                        metric, _, inst_df, _, conc = subset_metrics(f_events, c, "label_self_relative", family, split, fold_id)
                        fold_anchor_values = []
                        anchor_valid_counts = []
                        for anchor in range(horizon):
                            fa = f_events.loc[(f_events["daily_trading_calendar_index"].astype(int) % horizon) == anchor]
                            am, _, _, _, _ = subset_metrics(fa, c, "label_self_relative", family, split, fold_id, anchor)
                            fold_anchor_values.append(am["mean_spread"])
                            anchor_valid_counts.append(am["valid_signal_date_count"])
                        fold_anchor_s = pd.Series(fold_anchor_values).replace([np.inf, -np.inf], np.nan).dropna()
                        fold_anchor_mean = safe_mean(fold_anchor_s)
                    ...
                    fold_rows.append(
                        {
                            ...
                            "fold_top5_instrument_contribution_share": conc["top5_instrument_contribution_share"],
                            "fold_top1_industry_contribution_share": conc["top1_industry_contribution_share"],
                            "fold_contribution_share_of_total_abs_contribution": conc["fold_contribution_share_of_total_abs_contribution"],
                        }
                    )
                    sample_rows.append(
                        {
                    ...
                    conc_rows.append(dict(conc, horizon=h, scope="fold_full_daily"))
                fdf = pd.DataFrame([r for r in fold_rows if r["family"] == family and r["horizon"] == h and r["split"] == split])
                sdf = pd.DataFrame([r for r in sample_rows if r["family"] == family and r["horizon"] == h and r["split"] == split])
                if not fdf.empty:
                    spread_s = fdf["fold_anchor_controlled_mean_spread"].replace([np.inf, -np.inf], np.nan).dropna()
                    pis_s = fdf["fold_positive_instrument_share"].replace([np.inf, -np.inf], np.nan).dropna()
                    mono_s = fdf["fold_decile_monotonicity_score"].replace([np.inf, -np.inf], np.nan).dropna()
                    fold_disp_rows.append(
                        {
                            ...
                            "horizon": h,
                            "split": split,
                            "evaluable_fold_count": int(sdf["fold_evaluable_flag"].map(bool_value).sum()) if not sdf.empty else 0,
                            "positive_fold_count": int((spread_s > 0).sum()),
                            "median_fold_spread": safe_median(spread_s),
                            "min_fold_spread": float(spread_s.min()) if len(spread_s) else np.nan,
                            "fold_positive_instrument_share_median": safe_median(pis_s),
                            ...
                            "max_fold_top1_instrument_contribution_share": fdf["fold_top1_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max(),
                            "max_fold_top5_instrument_contribution_share": fdf["fold_top5_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max(),
                            "max_fold_contribution_share_of_total_abs_contribution": fdf["fold_contribution_share_of_total_abs_contribution"].replace([np.inf, -np.inf], np.nan).max(),
                        }
                    )
            for split in ["validation_oof_unseen", "robustness_oof_unseen"]:
                row = next((r for r in anchor_rows if r["family"] == family and r["horizon"] == h and r["split"] == split), None)
                if row is not None:
                    time_rows.append(row.copy())
        for split in ["validation_oof_unseen", "robustness_oof_unseen"]:
            vals = {}
    ...
    for row in anchor_rows:
        if row["horizon"] == "H3" and row["split"] in ["validation_oof_unseen", "robustness_oof_unseen"]:
            family_comparison_rows.append(
                {
                    "family": row["family"],
                    "split": row["split"],
                    "H3_anchor_controlled_mean_spread": row["anchor_controlled_mean_spread"],
                    "H3_anchor_controlled_positive_instrument_share": row["anchor_controlled_positive_instrument_share"],
                    "family_winner_selected": False,
                    "cross_family_score_formed": False,
                }
            )
    frames = {
        "full": pd.DataFrame(full_rows),
...
def write_metric_artifacts_multi(paths: R083Paths, frames: dict[str, pd.DataFrame]) -> None:
    h3 = lambda df: df.loc[df["horizon"].eq("H3")] if not df.empty else df
    write_csv(h3(frames["full"]), paths.metrics_dir / "r08_3_h3_full_daily_oof_spread_by_family.csv")
    write_csv(h3(frames["anchor"]), paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread_by_family.csv")
    write_csv(h3(frames["anchor"]).loc[h3(frames["anchor"])["split"].eq("train_oof_unseen")] if not frames["anchor"].empty else frames["anchor"], paths.metrics_dir / "r08_3_h3_train_baseline_summary_by_family.csv")
    write_csv(h3(frames["fold"]), paths.metrics_dir / "r08_3_h3_fold_unseen_state_spread_by_family.csv")
    write_csv(h3(frames["dispersion"]), paths.metrics_dir / "r08_3_h3_fold_dispersion_summary_by_family.csv")
    write_csv(h3(frames["instrument"]), paths.metrics_dir / "r08_3_h3_instrument_transfer_summary_by_family.csv")
    write_csv(h3(frames["time"]), paths.metrics_dir / "r08_3_h3_time_transfer_summary_by_family.csv")
    write_csv(h3(frames["year"]), paths.metrics_dir / "r08_3_h3_year_availability_and_positive_count_by_family.csv")
    write_csv(h3(frames["decile"]), paths.metrics_dir / "r08_3_h3_decile_monotonicity_by_family_anchor.csv")
    write_csv(h3(frames["concentration"]), paths.metrics_dir / "r08_3_h3_concentration_summary_by_family.csv")
    write_csv(frames["full"].loc[frames["full"]["horizon"].eq("H5")] if not frames["full"].empty else frames["full"], paths.metrics_dir / "r08_3_h5_diagnostic_oof_spread_by_family.csv")
    write_csv(frames["full"].loc[frames["full"]["horizon"].eq("H10")] if not frames["full"].empty else frames["full"], paths.metrics_dir / "r08_3_h10_diagnostic_oof_spread_by_family.csv")
    write_csv(frames["horizon_shape"], paths.metrics_dir / "r08_3_horizon_shape_summary_by_family.csv")
    write_csv(frames["overlap"], paths.metrics_dir / "r08_3_overlap_adjusted_confidence_summary_by_family.csv")
    write_csv(frames["family_comparison"], paths.metrics_dir / "r08_3_family_comparison_audit.csv")
    write_csv(frames["sample"], paths.audit_dir / "r08_3_fold_sample_audit.csv")
    write_csv(frames["concentration"], paths.audit_dir / "r08_3_concentration_audit.csv")


def row_for_family(df: pd.DataFrame, family: str, horizon: str, split: str) -> pd.Series | None:
    if df.empty:
        return None
    sub = df.loc[df["family"].eq(family) & df["horizon"].eq(horizon) & df["split"].eq(split)]
    return sub.iloc[0] if len(sub) else None


def split_evaluable_year_count(availability: pd.DataFrame, split: str, horizon: str) -> int:
    sub = availability.loc[availability["split"].eq(split) & availability["horizon"].eq(horizon)]
    return int(sub.iloc[0]["split_actual_evaluable_year_count"]) if len(sub) else 0


def concentration_gate_family(config: dict[str, Any], conc: pd.DataFrame, family: str) -> tuple[bool, dict[str, Any]]:
    c = config["frozen_formula_constants"]
    h3 = conc.loc[conc["family"].eq(family) & conc["horizon"].eq("H3")] if not conc.empty else pd.DataFrame()
    val = h3.loc[h3["split"].eq("validation_oof_unseen") & h3["scope"].eq("aggregate_anchor_controlled")]
    rob = h3.loc[h3["split"].eq("robustness_oof_unseen") & h3["scope"].eq("aggregate_anchor_controlled")]
    fold = h3.loc[h3["scope"].eq("fold_full_daily") & h3["split"].isin(["validation_oof_unseen", "robustness_oof_unseen"])]
    anchor = h3.loc[h3["scope"].eq("aggregate_anchor")]
    ...
    fold_pass = (
        ...
        and fold["top1_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max() <= float(c["max_fold_top1_instrument_contribution_share_max"])
        and fold["top5_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max() <= float(c["max_fold_top5_instrument_contribution_share_max"])
        and fold["fold_contribution_share_of_total_abs_contribution"].replace([np.inf, -np.inf], np.nan).max() <= float(c["max_fold_contribution_share_of_total_abs_contribution_max"])
    )
    max_fold_top1 = fold["top1_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max() if len(fold) else np.nan
    max_fold_top5 = fold["top5_instrument_contribution_share"].replace([np.inf, -np.inf], np.nan).max() if len(fold) else np.nan
    max_fold_share = fold["fold_contribution_share_of_total_abs_contribution"].replace([np.inf, -np.inf], np.nan).max() if len(fold) else np.nan
    max_anchor_share = np.nan
    if len(anchor):
        vals = anchor["contribution_denominator"].replace([np.inf, -np.inf], np.nan).fillna(0.0)
        total = vals.sum()
        max_anchor_share = float(vals.max() / total) if total else np.nan
    anchor_pass = finite(max_anchor_share) and max_anchor_share <= float(c["max_anchor_abs_contribution_share_of_total_max"])
    out = agg_pass(val) and agg_pass(rob) and fold_pass and anchor_pass
    return out, {"max_fold_top1": max_fold_top1, "max_fold_top5": max_fold_top5, "max_fold_share": max_fold_share, "max_anchor_share": max_anchor_share}


def build_horizon_diagnostic_inputs_multi(config: dict[str, Any], anchor: pd.DataFrame, conc: pd.DataFrame) -> pd.DataFrame:
    c = config["frozen_formula_constants"]
    rows = []
    for family in PRIMARY_FAMILIES:
        for horizon in ["H5", "H10"]:
            val = row_for_family(anchor, family, horizon, "validation_oof_unseen")
            rob = row_for_family(anchor, family, horizon, "robustness_oof_unseen")
            hnum = int(horizon[1:])
            hconc = conc.loc[conc["family"].eq(family) & conc["horizon"].eq(horizon) & conc["scope"].eq("aggregate_anchor_controlled")] if not conc.empty else pd.DataFrame()
            val_conc = hconc.loc[hconc["split"].eq("validation_oof_unseen")]
            rob_conc = hconc.loc[hconc["split"].eq("robustness_oof_unseen")]
            val_top1 = float(val_conc.iloc[0]["top1_instrument_contribution_share"]) if len(val_conc) else np.nan
            rob_top1 = float(rob_conc.iloc[0]["top1_instrument_contribution_share"]) if len(rob_conc) else np.nan
            ...
            diag = (
                ...
                and rob is not None
                and val["anchor_controlled_mean_spread"] > float(c["diagnostic_horizon_validation_mean_spread_min"])
                and val["anchor_controlled_median_spread"] >= float(c["diagnostic_horizon_validation_median_spread_min"])
                and rob["anchor_controlled_mean_spread"] > float(c["diagnostic_horizon_robustness_mean_spread_min"])
                and rob["anchor_controlled_median_spread"] >= float(c["diagnostic_horizon_robustness_median_spread_min"])
                and val["anchor_controlled_positive_instrument_share"] >= float(c["diagnostic_horizon_validation_positive_instrument_share_min"])
                and rob["anchor_controlled_positive_instrument_share"] >= float(c["diagnostic_horizon_robustness_positive_instrument_share_min"])
                and val["anchor_controlled_decile_monotonicity_score"] >= float(c["diagnostic_horizon_monotonicity_min"])
                and rob["anchor_controlled_decile_monotonicity_score"] >= float(c["diagnostic_horizon_monotonicity_min"])
            ...
            robustness_not_collapsed = (
                val is not None
                and rob is not None
                and val["anchor_controlled_mean_spread"] > 0
                and rob["anchor_controlled_mean_spread"] >= -0.0025
                and rob["anchor_controlled_mean_spread"] <= 0
            )
            rows.append(
                {
                    "family": family,
                    "horizon": horizon,
                    "validation_mean_spread": getattr(val, "anchor_controlled_mean_spread", np.nan) if val is not None else np.nan,
                    "validation_median_spread": getattr(val, "anchor_controlled_median_spread", np.nan) if val is not None else np.nan,
                    "robustness_mean_spread": getattr(rob, "anchor_controlled_mean_spread", np.nan) if rob is not None else np.nan,
                    "robustness_median_spread": getattr(rob, "anchor_controlled_median_spread", np.nan) if rob is not None else np.nan,
                    "validation_positive_instrument_share": getattr(val, "anchor_controlled_positive_instrument_share", np.nan) if val is not None else np.nan,
                    "robustness_positive_instrument_share": getattr(rob, "anchor_controlled_positive_instrument_share", np.nan) if rob is not None else np.nan,
                    "validation_anchor_decile_monotonicity": getattr(val, "anchor_controlled_decile_monotonicity_score", np.nan) if val is not None else np.nan,
                    "robustness_anchor_decile_monotonicity": getattr(rob, "anchor_controlled_decile_monotonicity_score", np.nan) if rob is not None else np.nan,
                    ...
                    "diagnostic_concentration_denominator_zero_flag": denom_zero,
                    "diagnostic_horizon_positive": diag,
                    "diagnostic_horizon_validation_positive_robustness_not_collapsed": robustness_not_collapsed,
                }
            )
    return pd.DataFrame(rows)


def build_gate_inputs_multi(
    config: dict[str, Any],
    paths: R083Paths,
    candidates: pd.DataFrame,
    scope_df: pd.DataFrame,
    ...
    c = config["frozen_formula_constants"]
    rows = []
    diagnostic_inputs = build_horizon_diagnostic_inputs_multi(config, frames["anchor"], frames["concentration"])
    daily_counts = candidates.groupby("split")["signal_date"].nunique().to_dict()
    panel_sample_pass = (
        candidates["instrument_id"].nunique() >= int(c["full_scope_instrument_count_min"])
        and daily_counts.get("train", 0) >= int(c["daily_signal_date_count_train_min"])
    ...
    for family in PRIMARY_FAMILIES:
        ...
        sample = frames["sample"]
        year = frames["year"]
        val = row_for_family(anchor, family, "H3", "validation_oof_unseen")
        rob = row_for_family(anchor, family, "H3", "robustness_oof_unseen")
        train = row_for_family(anchor, family, "H3", "train_oof_unseen")
        val_full = row_for_family(full, family, "H3", "validation_oof_unseen")
        rob_full = row_for_family(full, family, "H3", "robustness_oof_unseen")
        val_disp = row_for_family(disp, family, "H3", "validation_oof_unseen")
        rob_disp = row_for_family(disp, family, "H3", "robustness_oof_unseen")
        f_scope = scope_df.loc[scope_df["family"].eq(family)]
        b_scope = bucket_df.loc[bucket_df["family"].eq(family)]
        scope_pass = bool(f_scope["family_scope_pass"].map(bool_value).all()) if len(f_scope) else False
        bucket_pass = bool(b_scope["bucket_edge_valid"].map(bool_value).all()) if len(b_scope) else False
        scope_ref = scope_audit.loc[scope_audit["family"].eq(family)]
        unexplained_scope_change = bool(scope_ref["unexplained_family_scope_reference_count_changed"].map(bool_value).any()) if len(scope_ref) else True
        val_sample = sample.loc[sample["family"].eq(family) & sample["horizon"].eq("H3") & sample["split"].eq("validation_oof_unseen")]
        rob_sample = sample.loc[sample["family"].eq(family) & sample["horizon"].eq("H3") & sample["split"].eq("robustness_oof_unseen")]
        val_evaluable_fold_count = int(val_sample["fold_evaluable_flag"].map(bool_value).sum()) if len(val_sample) else 0
        rob_evaluable_fold_count = int(rob_sample["fold_evaluable_flag"].map(bool_value).sum()) if len(rob_sample) else 0
        val_dates = int(getattr(val, "anchor_controlled_valid_signal_date_count", 0)) if val is not None else 0
        rob_dates = int(getattr(rob, "anchor_controlled_valid_signal_date_count", 0)) if rob is not None else 0
        val_full_count = int(getattr(val, "anchor_controlled_full_valid_instrument_count", 0)) if val is not None else 0
        rob_full_count = int(getattr(rob, "anchor_controlled_full_valid_instrument_count", 0)) if rob is not None else 0
        aggregate_floors_pass = (
            val_full_count >= int(c["aggregate_oof_full_valid_instrument_count_min"])
            and rob_full_count >= int(c["aggregate_oof_full_valid_instrument_count_min"])
            and val_dates >= int(c["aggregate_oof_full_daily_valid_signal_date_count_min"])
            and rob_dates >= int(c["aggregate_oof_full_daily_valid_signal_date_count_min"])
        )
        val_overlap = frames["overlap"].loc[frames["overlap"]["family"].eq(family) & frames["overlap"]["horizon"].eq("H3") & frames["overlap"]["split"].eq("validation_oof_unseen")] if not frames["overlap"].empty else pd.DataFrame()
        rob_overlap = frames["overlap"].loc[frames["overlap"]["family"].eq(family) & frames["overlap"]["horizon"].eq("H3") & frames["overlap"]["split"].eq("robustness_oof_unseen")] if not frames["overlap"].empty else pd.DataFrame()
        val_anchor_evaluable_count = int((val_overlap["anchor_valid_signal_date_count"] >= int(c["h3_anchor_min_valid_signal_date_count_min"])).sum()) if len(val_overlap) else 0
        rob_anchor_evaluable_count = int((rob_overlap["anchor_valid_signal_date_count"] >= int(c["h3_anchor_min_valid_signal_date_count_min"])).sum()) if len(rob_overlap) else 0
        val_mean = getattr(val, "anchor_controlled_mean_spread", np.nan) if val is not None else np.nan
        train_mean = getattr(train, "anchor_controlled_mean_spread", np.nan) if train is not None else np.nan
        val_pos_inst = getattr(val, "anchor_controlled_positive_instrument_share", 0.0) if val is not None else 0.0
        val_pos_fold_count = int(getattr(val_disp, "positive_fold_count", 0)) if val_disp is not None else 0
        ...
        if panel_sample_pass and val_evaluable_fold_count == 5 and rob_evaluable_fold_count == 5 and aggregate_floors_pass and val_anchor_evaluable_count == 3 and rob_anchor_evaluable_count == 3:
            sample_status = "pass"
            fold_coverage_caveat = False
        elif panel_sample_pass and val_evaluable_fold_count == 4 and rob_evaluable_fold_count >= 4 and aggregate_floors_pass and caveat_margin:
            sample_status = "pass_with_fold_coverage_caveat"
            ...
            sample_status = "fail"
            fold_coverage_caveat = False
        val_years = year.loc[year["family"].eq(family) & year["horizon"].eq("H3") & year["split"].eq("validation_oof_unseen") & year["fold_id"].astype(str).eq("aggregate")] if not year.empty else pd.DataFrame()
        rob_years = year.loc[year["family"].eq(family) & year["horizon"].eq("H3") & year["split"].eq("robustness_oof_unseen") & year["fold_id"].astype(str).eq("aggregate")] if not year.empty else pd.DataFrame()
        val_positive_year_count = int(val_years["year_positive_flag"].map(bool_value).sum()) if len(val_years) else 0
        rob_positive_year_count = int(rob_years["year_positive_flag"].map(bool_value).sum()) if len(rob_years) else 0
        validation_year_count = split_evaluable_year_count(availability, "validation", "H3")
        robustness_year_count = split_evaluable_year_count(availability, "robustness", "H3")
        rob_mean = getattr(rob, "anchor_controlled_mean_spread", np.nan) if rob is not None else np.nan
        val_median = getattr(val, "anchor_controlled_median_spread", np.nan) if val is not None else np.nan
        rob_median = getattr(rob, "anchor_controlled_median_spread", np.nan) if rob is not None else np.nan
        val_non_det = finite(val_mean) and finite(train_mean) and val_mean >= train_mean - float(c["validation_train_spread_tolerance"])
        rob_non_det = finite(rob_mean) and finite(train_mean) and rob_mean >= train_mean - float(c["robustness_train_spread_tolerance"])
        validation_single_positive_year_caveat = val_positive_year_count == 1
        time_gate = (
            ...
            and val_median >= float(c["validation_median_state_spread_min"])
            and getattr(val, "anchor_positive_offset_count", 0) >= int(c["positive_anchor_offset_count_validation_min"])
            and val_positive_year_count == validation_year_count
            and val_non_det
            and not bool_value(getattr(val, "full_daily_anchor_sign_conflict_flag", True))
            and finite(getattr(val_full, "h3_full_daily_mean_spread", np.nan))
            and getattr(val_full, "h3_full_daily_mean_spread", np.nan) >= float(c["validation_full_daily_mean_spread_floor"])
            ...
            and rob_median >= float(c["robustness_median_state_spread_min"])
            and getattr(rob, "anchor_positive_offset_count", 0) >= int(c["positive_anchor_offset_count_robustness_min"])
            and rob_positive_year_count == robustness_year_count
            and rob_non_det
            and not bool_value(getattr(rob, "full_daily_anchor_sign_conflict_flag", True))
            and finite(getattr(rob_full, "h3_full_daily_mean_spread", np.nan))
            and getattr(rob_full, "h3_full_daily_mean_spread", np.nan) >= float(c["robustness_full_daily_mean_spread_floor"])
        )
        instrument_gate = val_pos_inst >= float(c["positive_instrument_share_validation_min"]) and (getattr(rob, "anchor_controlled_positive_instrument_share", 0.0) if rob is not None else 0.0) >= float(c["positive_instrument_share_robustness_min"])
        fold_stability_gate = (
            val_pos_fold_count >= int(c["positive_fold_count_validation_min"])
            and int(getattr(rob_disp, "positive_fold_count", 0)) >= int(c["positive_fold_count_robustness_min"])
            and getattr(val_disp, "median_fold_spread", np.nan) > float(c["median_fold_spread_validation_min"])
        ...
        conc_gate, conc_detail = concentration_gate_family(config, frames["concentration"], family)
        hdiag = diagnostic_inputs.loc[diagnostic_inputs["family"].eq(family)]
        h5_pass = bool_value(hdiag.loc[hdiag["horizon"].eq("H5"), "diagnostic_horizon_positive"].iloc[0]) if len(hdiag.loc[hdiag["horizon"].eq("H5")]) else False
        h10_pass = bool_value(hdiag.loc[hdiag["horizon"].eq("H10"), "diagnostic_horizon_positive"].iloc[0]) if len(hdiag.loc[hdiag["horizon"].eq("H10")]) else False
        cleanliness_failed = not mono_gate or not conc_gate
        spread_positive = finite(val_mean) and val_mean > 0 and finite(rob_mean) and rob_mean > 0 and instrument_gate
        no_disallowed = (not unexplained_scope_change) and conc_gate and sample_status == "pass"
        rows.append(
            {
                "family": family,
                "primary_score_formed_flag": scope_pass,
                "daily_panel_sample_gate_pass": panel_sample_pass,
                "aggregate_oof_sample_status": sample_status,
                "fold_coverage_caveat": fold_coverage_caveat,
                "validation_evaluable_fold_count": val_evaluable_fold_count,
                ...
                "H3_concentration_gate_pass": conc_gate,
                "H3_robustness_non_deterioration_pass": rob_non_det,
                "validation_actual_evaluable_year_count_H3": validation_year_count,
                "robustness_actual_evaluable_year_count_H3": robustness_year_count,
                "validation_single_positive_year_caveat": validation_single_positive_year_caveat,
                "validation_single_positive_year_blocks_support": validation_single_positive_year_caveat,
                "no_disallowed_caveat_active": no_disallowed,
                "unexplained_family_scope_reference_count_changed": unexplained_scope_change,
                "retained_factor_set_comparability_pass": bool(f_scope["retained_factor_set_comparability_pass"].map(bool_value).all()) if len(f_scope) else False,
                "bucket_edge_valid_all_folds": bucket_pass,
                "H3_train_oof_anchor_controlled_mean_spread": train_mean,
                "H3_validation_anchor_controlled_mean_spread": val_mean,
                "H3_validation_anchor_controlled_median_spread": val_median,
                "H3_robustness_anchor_controlled_mean_spread": rob_mean,
                ...
                "H3_max_fold_top1_instrument_contribution_share": conc_detail.get("max_fold_top1", np.nan),
                "H3_max_fold_top5_instrument_contribution_share": conc_detail.get("max_fold_top5", np.nan),
                "H3_max_fold_contribution_share_of_total_abs_contribution": conc_detail.get("max_fold_share", np.nan),
                "H3_max_anchor_abs_contribution_share_of_total": conc_detail.get("max_anchor_share", np.nan),
                "daily_observation_spread_positive_but_cleanliness_failed": bool(spread_positive and cleanliness_failed),
                "H5_diagnostic_horizon_positive": h5_pass,
                "H10_diagnostic_horizon_positive": h10_pass,
                "authorized_strategy_requirement": False,
            }
    ...
    write_csv(gate, paths.decision_dir / "r08_3_gate_inputs_by_family.csv")
    write_csv(diagnostic_inputs, paths.decision_dir / "r08_3_horizon_diagnostic_inputs_by_family.csv")
    return gate, diagnostic_inputs


def build_final_decision_multi(paths: R083Paths, gate: pd.DataFrame, diagnostic: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    replay_rows = []
    final_rows = []
    for rec in gate.to_dict("records"):
        family = rec["family"]
        sample_status = rec.get("aggregate_oof_sample_status", "fail")
        sample_fail = sample_status == "fail"
        sample_pass = sample_status == "pass"
        sample_caveat = sample_status == "pass_with_fold_coverage_caveat"
        score_missing = not bool_value(rec.get("primary_score_formed_flag", False))
        time_pass = bool_value(rec.get("H3_time_transfer_gate_pass", False))
        inst_pass = bool_value(rec.get("H3_instrument_transfer_gate_pass", False))
        fold_pass = bool_value(rec.get("H3_fold_stability_gate_pass", False))
        anchor_pass = bool_value(rec.get("H3_anchor_stability_gate_pass", False))
        mono_pass = bool_value(rec.get("H3_monotonicity_gate_pass", False))
        conc_pass = bool_value(rec.get("H3_concentration_gate_pass", False))
        rob_nd_pass = bool_value(rec.get("H3_robustness_non_deterioration_pass", False))
        no_caveat = bool_value(rec.get("no_disallowed_caveat_active", False))
        all_non_sample = time_pass and inst_pass and fold_pass and anchor_pass and mono_pass and conc_pass and rob_nd_pass
        support_all = sample_pass and all_non_sample and no_caveat
        hdiag = bool(diagnostic.loc[diagnostic["family"].eq(family), "diagnostic_horizon_positive"].map(bool_value).any()) if len(diagnostic) else False
        rules = [
            ("rule_F_01", "family scope, direction sample, retained factor, factor-set comparability, bucket-edge, or H3 aggregate sample fails", score_missing or sample_fail, "r08_3_family_blocked_scope_or_sample_insufficient"),
            ("rule_F_02", "aggregate_oof_sample_status = pass_with_fold_coverage_caveat and all non-sample H3 support gates pass", sample_caveat and all_non_sample, "r08_3_daily_family_h3_sample_limited_candidate"),
            ("rule_F_03", "H3 support gates pass except fold stability", sample_pass and time_pass and inst_pass and anchor_pass and mono_pass and conc_pass and rob_nd_pass and not fold_pass, "r08_3_daily_family_h3_fold_fragile_candidate"),
            ("rule_F_04", "time transfer passes, instrument transfer fails, other H3 cleanliness gates pass", sample_pass and time_pass and not inst_pass and fold_pass and anchor_pass and mono_pass and conc_pass and rob_nd_pass, "r08_3_daily_family_h3_time_transfer_only"),
            ("rule_F_05", "all H3 support gates pass", support_all, "r08_3_daily_family_h3_transferability_diagnostic_supported"),
            ("rule_F_06", "H3 sample passes and H3 support fails, while H5/H10 diagnostic horizon passes", sample_pass and not support_all and hdiag, "r08_3_family_horizon_mismatch_diagnostic_only"),
            ("rule_F_07", "otherwise", True, "r08_3_no_daily_family_h3_transferability_support"),
        ]
        selected = ""
        seen = False
        for rule_id, text, condition, decision in rules:
            raw = bool(condition)
            is_selected = (not seen) and raw
            if is_selected:
                selected = decision
                seen = True
            replay_rows.append(
                {
                    ...
                    "selected_rule_flag": is_selected,
        ...
        final_rows.append(
            {
                "family": family,
                "per_family_final_decision": selected,
                "authorized_strategy_requirement": False,
                "allowed_next_requirement": "confirmatory_daily_family_h3_transferability_diagnostic" if selected == "r08_3_daily_family_h3_transferability_diagnostic_supported" else "",
                "primary_horizon": "H3",
                "diagnostic_horizons": "H5;H10",
                "aggregate_oof_sample_status": sample_status,
                "H3_validation_anchor_controlled_mean_spread": rec.get("H3_validation_anchor_controlled_mean_spread", np.nan),
                "H3_robustness_anchor_controlled_mean_spread": rec.get("H3_robustness_anchor_controlled_mean_spread", np.nan),
                "H3_validation_anchor_controlled_positive_instrument_share": rec.get("H3_validation_anchor_controlled_positive_instrument_share", np.nan),
                "H3_robustness_anchor_controlled_positive_instrument_share": rec.get("H3_robustness_anchor_controlled_positive_instrument_share", np.nan),
                "daily_observation_spread_positive_but_cleanliness_failed": rec.get("daily_observation_spread_positive_but_cleanliness_failed", False),
            }
        )
    per_replay = pd.DataFrame(replay_rows)
    per_final = pd.DataFrame(final_rows)
    decisions = per_final["per_family_final_decision"].tolist() if len(per_final) else []
    aggregate_rules = [
        ("aggregate_rule_01", "global data/execution contract failure", False, "r08_3_blocked_data_or_execution_contract"),
        ("aggregate_rule_02", "all families are sample blocked", bool(decisions) and all(x == "r08_3_family_blocked_scope_or_sample_insufficient" for x in decisions), "r08_3_all_families_sample_blocked"),
        ("aggregate_rule_03", "any family is H3 supported", any(x == "r08_3_daily_family_h3_transferability_diagnostic_supported" for x in decisions), "r08_3_at_least_one_family_h3_transferability_diagnostic_supported"),
        ("aggregate_rule_04", "no family supported and any family sample-limited", not any(x == "r08_3_daily_family_h3_transferability_diagnostic_supported" for x in decisions) and any(x == "r08_3_daily_family_h3_sample_limited_candidate" for x in decisions), "r08_3_some_family_sample_limited_candidate"),
        ("aggregate_rule_05", "no family supported and any family horizon mismatch", not any(x == "r08_3_daily_family_h3_transferability_diagnostic_supported" for x in decisions) and any(x == "r08_3_family_horizon_mismatch_diagnostic_only" for x in decisions), "r08_3_some_family_horizon_mismatch_diagnostic_only"),
        ("aggregate_rule_06", "otherwise", True, "r08_3_no_family_h3_transferability_support"),
    ]
    agg_replay_rows = []
    agg_selected = ""
    seen = False
    for rule_id, text, condition, decision in aggregate_rules:
        raw = bool(condition)
        is_selected = (not seen) and raw
        if is_selected:
            seen = True
            agg_selected = decision
        agg_replay_rows.append({"rule_id": rule_id, "rule_condition_text": text, "raw_condition_met": raw, "selected_rule_flag": is_selected, "decision_if_selected": decision})
    aggregate_final = pd.DataFrame(
        [
            {
                "aggregate_r08_3_final_decision": agg_selected,
                "authorized_strategy_requirement": False,
                "allowed_next_requirement": "confirmatory_daily_family_h3_transferability_diagnostic" if "supported" in agg_selected else "",
                "tested_family_count": len(PRIMARY_FAMILIES),
                "all_pre_registered_families_reported": set(per_final["family"]) == set(PRIMARY_FAMILIES) if len(per_final) else False,
                "no_family_winner_selection": True,
                "no_cross_family_score": True,
            }
        ]
    )
    aggregate_replay = pd.DataFrame(agg_replay_rows)
    write_csv(per_replay, paths.decision_dir / "r08_3_per_family_final_decision_replay.csv")
    write_csv(per_final, paths.decision_dir / "r08_3_per_family_final_decision.csv")
    write_csv(aggregate_replay, paths.decision_dir / "r08_3_aggregate_final_decision_replay.csv")
    write_csv(aggregate_final, paths.decision_dir / "r08_3_aggregate_final_decision.csv")
    return per_replay, per_final, aggregate_replay, aggregate_final


def write_report_multi(paths: R083Paths, gate: pd.DataFrame, diagnostic: pd.DataFrame, per_final: pd.DataFrame, aggregate_final: pd.DataFrame) -> None:
    h3_anchor = pd.read_csv(paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread_by_family.csv")
    h3_fold = pd.read_csv(paths.metrics_dir / "r08_3_h3_fold_dispersion_summary_by_family.csv")
    hshape = pd.read_csv(paths.metrics_dir / "r08_3_horizon_shape_summary_by_family.csv")
    agg_decision = aggregate_final.iloc[0]["aggregate_r08_3_final_decision"] if len(aggregate_final) else ""
    lines = [
        "# R08.3 Daily-Observed Volume/Rank Families H3/H5/H10 Transferability Diagnostic Report",
        "",
        "## 1. 结论",
        "",
        f"`aggregate_r08_3_final_decision = {agg_decision}`",
        "",
        "`authorized_strategy_requirement = false`。R08.3 是 diagnostic-only audit，不构造策略、组合、top-N、top20%、family winner 或 production signal。",
        "",
        "Per-family decisions:",
        "",
        "| family | decision | H3 validation spread | H3 robustness spread | sample status |",
        "|:--|:--|--:|--:|:--|",
    ]
    for rec in per_final.itertuples(index=False):
        lines.append(
            f"| {rec.family} | `{rec.per_family_final_decision}` | {pct_text(rec.H3_validation_anchor_controlled_mean_spread)} | {pct_text(rec.H3_robustness_anchor_controlled_mean_spread)} | `{rec.aggregate_oof_sample_status}` |"
        )
    lines.extend(["", "## 2. H3 Anchor-Controlled Readout", "", "| family | split | mean spread | median spread | positive anchors | positive inst share | valid instruments | valid dates | monotonicity | conflict |", "|:--|:--|--:|--:|--:|--:|--:|--:|--:|:--|"])
    for rec in h3_anchor.itertuples(index=False):
        lines.append(
            f"| {rec.family} | {rec.split} | {pct_text(rec.anchor_controlled_mean_spread)} | {pct_text(rec.anchor_controlled_median_spread)} | {int(rec.anchor_positive_offset_count)} | {pct_text(rec.anchor_controlled_positive_instrument_share)} | {int(rec.anchor_controlled_full_valid_instrument_count)} | {int(rec.anchor_controlled_valid_signal_date_count)} | {num_text(rec.anchor_controlled_decile_monotonicity_score)} | `{bool_value(rec.full_daily_anchor_sign_conflict_flag)}` |"
        )
    lines.extend(["", "## 3. Gate Replay", "", "| family | time | instrument | fold | anchor | monotonicity | concentration | non-deterioration | caveat |", "|:--|:--|:--|:--|:--|:--|:--|:--|:--|"])
    for rec in gate.itertuples(index=False):
        lines.append(
            f"| {rec.family} | `{bool_value(rec.H3_time_transfer_gate_pass)}` | `{bool_value(rec.H3_instrument_transfer_gate_pass)}` | `{bool_value(rec.H3_fold_stability_gate_pass)}` | `{bool_value(rec.H3_anchor_stability_gate_pass)}` | `{bool_value(rec.H3_monotonicity_gate_pass)}` | `{bool_value(rec.H3_concentration_gate_pass)}` | `{bool_value(rec.H3_robustness_non_deterioration_pass)}` | `{rec.aggregate_oof_sample_status}` |"
        )
    lines.extend(["", "## 4. Fold Dispersion", "", "| family | split | evaluable folds | positive folds | median spread | min spread | fold mono median | max fold top1 |", "|:--|:--|--:|--:|--:|--:|--:|--:|"])
    for rec in h3_fold.itertuples(index=False):
        lines.append(
            f"| {rec.family} | {rec.split} | {int(rec.evaluable_fold_count)} | {int(rec.positive_fold_count)} | {pct_text(rec.median_fold_spread)} | {pct_text(rec.min_fold_spread)} | {num_text(rec.fold_monotonicity_median)} | {pct_text(rec.max_fold_top1_instrument_contribution_share)} |"
        )
    lines.extend(["", "## 5. H5/H10 Diagnostic", "", "| family | horizon | val spread | robust spread | val inst share | robust inst share | diagnostic positive |", "|:--|:--|--:|--:|--:|--:|:--|"])
    for rec in diagnostic.itertuples(index=False):
        lines.append(
            f"| {rec.family} | {rec.horizon} | {pct_text(rec.validation_mean_spread)} | {pct_text(rec.robustness_mean_spread)} | {pct_text(rec.validation_positive_instrument_share)} | {pct_text(rec.robustness_positive_instrument_share)} | `{bool_value(rec.diagnostic_horizon_positive)}` |"
        )
    lines.extend(["", "## 6. Horizon Shape", "", "| family | split | H3 | H5 | H10 | sign pattern |", "|:--|:--|--:|--:|--:|:--|"])
    for rec in hshape.itertuples(index=False):
        lines.append(f"| {rec.family} | {rec.split} | {pct_text(rec.H3_mean_spread)} | {pct_text(rec.H5_mean_spread)} | {pct_text(rec.H10_mean_spread)} | `{rec.sign_pattern}` |")
    lines.extend(["", "## 7. Required Questions", ""])
    questions = [
        "1. R08.3 是否保持 diagnostic-only，且没有构造任何策略？是。",
        "2. 是否同步验证了三个 family？是，三者全部报告。",
        "3. 是否没有使用 `vwap_deviation` 作为 R08.3 primary family？是。",
        "4. 是否把 signal frequency 固定为 daily close-observed？是。",
        "5. 是否只把 H3 作为 primary horizon？是。",
        "6. H5/H10 是否只作为 diagnostic labels？是。",
        "7. daily signal panel 是否 PIT / as-of safe，且三个 family 共用同一 panel？是。",
        "8. daily factor percentile 是否使用 D-1 之前的 252 日 reference distribution？是。",
        "9. H3/H5/H10 self-relative labels 是否只使用 completed labels？是。",
        "10. daily overlapping label 是否被显式控制？是。",
        "11. H3 anchor offsets 是否全部可评价？见 fold sample / overlap audit。",
        "12. 5-fold instrument assignment 是否 deterministic 且 train 前冻结？是。",
        "13. direction 是否只来自 train years + seen folds + H3？是。",
        "14. H5/H10 是否没有参与 direction、bucket edge、factor retention？是。",
        "15. 每个 family 的 retained factor count 和 dropped factor list 是什么？见 family_scope_by_fold_audit。",
        "16. 每个 family 的 retained factor set 是否跨 fold 可比？见 family_scope_by_fold_audit。",
        "17. family scope reference count 是否发生变化？见 scope_audit。",
        "18. `rank_ts_rank_structure` 是否被明确标注 single-factor caveat？是。",
        "19. 每个 family/fold 的 bucket edge 是否有效？见 state_bucket_by_family_fold_audit。",
        "20. 每个 family 的 validation H3 anchor-controlled spread 是否为正？见 H3 table。",
        "21. 每个 family 的 robustness H3 anchor-controlled spread 是否严格为正且 median 非负？见 H3 table / gate inputs。",
        "22. 每个 family 的 full daily readout 是否与 anchor-controlled readout 冲突？见 conflict flag。",
        "23. 每个 family 的 validation / robustness H3 positive instrument share 是否达标？见 gate inputs。",
        "24. 每个 family 的 H3 fold stability 是否达标？见 gate replay。",
        "25. 每个 family 的 H3 anchor stability 是否达标？见 gate replay。",
        "26. 每个 family 的 H3 monotonicity 是否达标？见 gate replay。",
        "27. 每个 family 的 H3 concentration 是否达标？见 gate replay。",
        "28. 每个 family 的 H5/H10 diagnostic spread、monotonicity、positive instrument share、diagnostic concentration 是什么？见 H5/H10 table。",
        "29. 每个 family 的 horizon shape 是什么？见 horizon shape table。",
        "30. 是否确认没有按 validation / robustness 选择 winning family？是。",
        "31. 是否确认 `pass_with_fold_coverage_caveat` 没有被用于产生 supported decision？是。",
        "32. per-family final decisions 是什么？见结论表。",
        "33. aggregate R08.3 final decision 是什么？见结论。",
        "34. direction canonical sign 是否来自 H3 anchor-controlled train-seen stats，而不是 full daily overlapping stats？是。",
        "35. train OOF anchor-controlled baseline 是否落盘并用于 non-deterioration replay？是。",
        "36. 每个 family 是否存在 validation single-positive-year caveat，且该 caveat 是否阻断 support？见 gate inputs。",
        "37. 如果 daily spread / breadth 为正但 monotonicity / concentration 失败，是否标注 cleanliness failure？是。",
        "38. 是否允许写 strategy requirement？no。",
    ]
    lines.extend(questions)
    (paths.reports_dir / "r08_3_final_report.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")


def required_paths(paths: R083Paths) -> list[Path]:
    return [
        paths.audit_dir / "r08_3_run_manifest.json",
        paths.audit_dir / "r08_3_input_data_audit.csv",
        paths.audit_dir / "r08_3_daily_signal_panel_audit.csv",
        paths.audit_dir / "r08_3_data_availability_by_horizon_audit.csv",
        paths.audit_dir / "r08_3_scope_audit.csv",
        paths.audit_dir / "r08_3_family_factor_scope_audit.csv",
        paths.audit_dir / "r08_3_fold_assignment_audit.csv",
        paths.audit_dir / "r08_3_within_stock_normalization_audit.csv",
        paths.audit_dir / "r08_3_label_asof_audit.csv",
        paths.audit_dir / "r08_3_factor_direction_by_family_fold_audit.csv",
        paths.audit_dir / "r08_3_factor_nonconstant_observation_audit.csv",
        paths.audit_dir / "r08_3_family_scope_by_fold_audit.csv",
        paths.audit_dir / "r08_3_state_bucket_by_family_fold_audit.csv",
        paths.audit_dir / "r08_3_overlap_anchor_audit.csv",
        paths.audit_dir / "r08_3_fold_sample_audit.csv",
        paths.audit_dir / "r08_3_horizon_specific_instrument_validity_audit.csv",
        paths.audit_dir / "r08_3_concentration_audit.csv",
        paths.metrics_dir / "r08_3_h3_full_daily_oof_spread_by_family.csv",
        paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread_by_family.csv",
        paths.metrics_dir / "r08_3_h3_train_baseline_summary_by_family.csv",
        paths.metrics_dir / "r08_3_h3_fold_unseen_state_spread_by_family.csv",
        paths.metrics_dir / "r08_3_h3_fold_dispersion_summary_by_family.csv",
        paths.metrics_dir / "r08_3_h3_instrument_transfer_summary_by_family.csv",
        paths.metrics_dir / "r08_3_h3_time_transfer_summary_by_family.csv",
        paths.metrics_dir / "r08_3_h3_year_availability_and_positive_count_by_family.csv",
        paths.metrics_dir / "r08_3_h3_decile_monotonicity_by_family_anchor.csv",
        paths.metrics_dir / "r08_3_h3_concentration_summary_by_family.csv",
        paths.metrics_dir / "r08_3_h5_diagnostic_oof_spread_by_family.csv",
        paths.metrics_dir / "r08_3_h10_diagnostic_oof_spread_by_family.csv",
        paths.metrics_dir / "r08_3_horizon_shape_summary_by_family.csv",
        paths.metrics_dir / "r08_3_overlap_adjusted_confidence_summary_by_family.csv",
        paths.metrics_dir / "r08_3_family_comparison_audit.csv",
        paths.decision_dir / "r08_3_gate_inputs_by_family.csv",
        paths.decision_dir / "r08_3_horizon_diagnostic_inputs_by_family.csv",
        paths.decision_dir / "r08_3_per_family_final_decision_replay.csv",
        paths.decision_dir / "r08_3_per_family_final_decision.csv",
        paths.decision_dir / "r08_3_aggregate_final_decision_replay.csv",
        paths.decision_dir / "r08_3_aggregate_final_decision.csv",
        paths.reports_dir / "r08_3_final_report.md",
        paths.manifests_dir / "r08_3_artifact_hashes.json",
    ]
...
def run_pipeline(config_path: str | Path = DEFAULT_CONFIG) -> None:
    ...
    scope = family_factor_scope(inputs)
    factor_ids = scope_factor_ids(inputs)
    _, percentile, tie, tie_cluster, available_fids = build_factor_state_inputs(config, paths, inputs, candidates, factor_ids)
    build_scope_audits_multi(paths, inputs, candidates, scope, available_fids)
    scope_audit = pd.read_csv(paths.audit_dir / "r08_3_scope_audit.csv")
    build_normalization_audit_multi(paths, candidates, available_fids, percentile, tie, tie_cluster, factor_to_family_map(scope))
    _, _, directions = direction_by_family_fold(config, paths, candidates, label, scope, available_fids, percentile)
    events, scope_df, bucket_df, _ = build_oof_events_multi(config, paths, candidates, label, scope, available_fids, percentile, directions)
    frames = build_metrics_multi(config, paths, events)
    write_csv(frames["overlap"], paths.audit_dir / "r08_3_overlap_anchor_audit.csv")
    gate, diagnostic = build_gate_inputs_multi(config, paths, candidates, scope_df, bucket_df, frames, availability, scope_audit)
    _, per_final, _, aggregate_final = build_final_decision_multi(paths, gate, diagnostic)
    write_report_multi(paths, gate, diagnostic, per_final, aggregate_final)
    write_json(
        {
            ...
            "aggregate_final_decision": aggregate_final.iloc[0]["aggregate_r08_3_final_decision"],
            "authorized_strategy_requirement": bool_value(aggregate_final.iloc[0]["authorized_strategy_requirement"]),
        },
        paths.audit_dir / "r08_3_run_manifest.json",
    )
    write_json({"created_at": r01.now_iso(), "artifacts": artifact_hashes(paths)}, paths.manifests_dir / "r08_3_artifact_hashes.json")
...
def validate_outputs(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    aggregate_decision = ""
    if not missing:
        daily = pd.read_csv(paths.audit_dir / "r08_3_daily_signal_panel_audit.csv")
        fold = pd.read_csv(paths.audit_dir / "r08_3_fold_assignment_audit.csv")
        norm = pd.read_csv(paths.audit_dir / "r08_3_within_stock_normalization_audit.csv")
        label = pd.read_csv(paths.audit_dir / "r08_3_label_asof_audit.csv")
        direction = pd.read_csv(paths.audit_dir / "r08_3_factor_direction_by_family_fold_audit.csv")
        fam = pd.read_csv(paths.audit_dir / "r08_3_family_scope_by_fold_audit.csv")
        bucket = pd.read_csv(paths.audit_dir / "r08_3_state_bucket_by_family_fold_audit.csv")
        sample = pd.read_csv(paths.audit_dir / "r08_3_fold_sample_audit.csv")
        horizon_valid = pd.read_csv(paths.audit_dir / "r08_3_horizon_specific_instrument_validity_audit.csv")
        scope_audit = pd.read_csv(paths.audit_dir / "r08_3_scope_audit.csv")
        gate = pd.read_csv(paths.decision_dir / "r08_3_gate_inputs_by_family.csv")
        diag = pd.read_csv(paths.decision_dir / "r08_3_horizon_diagnostic_inputs_by_family.csv")
        per_replay = pd.read_csv(paths.decision_dir / "r08_3_per_family_final_decision_replay.csv")
        agg_replay = pd.read_csv(paths.decision_dir / "r08_3_aggregate_final_decision_replay.csv")
        per_final = pd.read_csv(paths.decision_dir / "r08_3_per_family_final_decision.csv")
        aggregate_final = pd.read_csv(paths.decision_dir / "r08_3_aggregate_final_decision.csv")
        report = (paths.reports_dir / "r08_3_final_report.md").read_text(encoding="utf-8")
        aggregate_decision = str(aggregate_final.iloc[0]["aggregate_r08_3_final_decision"])
        check("V03_primary_families_exact", config["scope"]["families"] == PRIMARY_FAMILIES, str(config["scope"]["families"]))
        check("V04_no_vwap_primary_family", "vwap_deviation" not in config["scope"]["families"], "")
        check("V05_primary_horizon_only_H3", config["scope"]["primary_horizon"] == "H3" and config["execution"]["primary_horizon"] == 3, "")
        check("V06_diagnostic_horizons_only_H5_H10", config["execution"]["diagnostic_horizons"] == [5, 10], "")
        check("V07_signal_frequency_daily", config["execution"]["signal_date_rule"] == "daily_close_observed", "")
        check("V08_weekly_signal_panel_not_used_as_primary", daily["weekly_signal_panel_not_used_as_primary"].map(bool_value).all(), "")
        check("V09_shared_daily_panel_across_families", pd.read_csv(paths.audit_dir / "r08_3_input_data_audit.csv")["shared_daily_panel_across_families"].map(bool_value).all(), "")
        check("V10_no_strategy_artifacts", not any(p.is_file() and any(x in p.name.lower() for x in ["portfolio", "backtest", "allocation", "top20", "top_"]) for p in paths.output_root.rglob("*")), "")
        check("V11_fold_assignment_sha256_mod5", set(fold["instrument_fold_id"].dropna().astype(int).unique()) == set(FOLD_IDS), "")
        sample_ids = fold.head(20)
        check("V12_fold_hash_replay", all(instrument_fold_id(x) == int(fid) for x, fid in zip(sample_ids["canonical_instrument_id"], sample_ids["instrument_fold_id"])), "")
        check("V13_all_5_folds_present", set(fam["fold_id"].dropna().astype(int).unique()) == set(FOLD_IDS), "")
        check("V14_no_fold_dropped_for_performance", set(sample["fold_id"].dropna().astype(int).unique()) == set(FOLD_IDS), "")
        check("V15_daily_trading_calendar_index_global_continuous", daily["daily_trading_calendar_index_global_continuous"].map(bool_value).all(), "")
        check("V16_direction_train_seen_only", direction["direction_source_split"].eq("train").all() and direction["direction_source_instrument_scope"].astype(str).str.contains("seen_folds_not_").all(), "")
        check("V17_direction_label_horizon_H3_only", direction["direction_label_horizon"].eq("H3").all(), "")
        check("V18_direction_canonical_sign_anchor_controlled", "factor_direction_stat_anchor_median" in direction.columns, "")
        check("V19_H5_H10_not_used_for_direction", direction["direction_label_horizon"].eq("H3").all(), "")
        check("V20_direction_anchor_stability_checked", "direction_anchor_stability_pass" in direction.columns, "")
        check("V21_retained_factor_set_comparability_checked", "retained_factor_set_comparability_pass" in fam.columns, "")
        check("V22_family_scope_reference_replayable", "unexplained_family_scope_reference_count_changed" in scope_audit.columns, "")
        check("V23_rank_ts_single_factor_caveat_present", scope_audit.loc[scope_audit["family"].eq("rank_ts_rank_structure"), "single_factor_family_caveat"].map(bool_value).all(), "")
        check("V24_bucket_edges_train_seen_only", bucket["frozen_before_validation_read"].map(bool_value).all() and bucket["bucket_edge_source_split"].eq("train").all(), "")
        check("V25_bucket_edge_validity_checked", "bucket_edge_valid" in bucket.columns, "")
        check("V26_within_stock_lookback_ends_D_minus_1", norm["within_stock_lookback_ends_at_D_minus_1"].map(bool_value).all(), "")
        check("V27_mid_rank_tie_handling_used", norm["mid_rank_tie_handling_used"].map(bool_value).all(), "")
        check("V28_self_relative_completed_labels_only", label["self_relative_labels_use_completed_labels_only"].map(bool_value).all(), "")
        check("V29_overlap_anchor_offsets_exist", (paths.metrics_dir / "r08_3_overlap_adjusted_confidence_summary_by_family.csv").exists(), "")
        check("V30_H3_primary_gate_uses_anchor_metrics", {"H3_time_transfer_gate_pass", "H3_anchor_stability_gate_pass"}.issubset(gate.columns), "")
        supported_families = set(
            per_final.loc[
                per_final["per_family_final_decision"].eq("r08_3_daily_family_h3_transferability_diagnostic_supported"),
                "family",
            ]
        )
        supported_gate = gate.loc[gate["family"].isin(supported_families)]
        support_positive_condition = supported_gate.empty or (
            (supported_gate["H3_validation_anchor_controlled_mean_spread"] > 0).all()
            and (supported_gate["H3_robustness_anchor_controlled_mean_spread"] > 0).all()
            and (supported_gate["H3_validation_anchor_controlled_median_spread"] >= 0).all()
            and (supported_gate["H3_robustness_anchor_controlled_median_spread"] >= 0).all()
        )
        check("V31_H3_support_requires_positive_validation_and_robustness", support_positive_condition, "")
        check("V32_full_daily_conflict_flags_exist", "full_daily_anchor_sign_conflict_flag" in pd.read_csv(paths.metrics_dir / "r08_3_h3_anchor_controlled_oof_spread_by_family.csv").columns, "")
        check("V33_train_oof_anchor_baseline_exists", (paths.metrics_dir / "r08_3_h3_train_baseline_summary_by_family.csv").exists(), "")
        check("V34_validation_single_positive_year_blocks_support", "validation_single_positive_year_blocks_support" in gate.columns, "")
        check("V35_H5_H10_diagnostic_only", diag["horizon"].isin(["H5", "H10"]).all(), "")
        check("V36_diagnostic_positive_requires_positive_robustness", not ((diag["diagnostic_horizon_positive"].map(bool_value)) & (diag["robustness_mean_spread"] <= 0)).any(), "")
        check("V37_horizon_switching_forbidden", per_final["authorized_strategy_requirement"].map(bool_value).eq(False).all() and aggregate_final["authorized_strategy_requirement"].map(bool_value).eq(False).all(), "")
        check("V38_partial_horizon_specific", horizon_valid["partial_instruments_horizon_specific"].map(bool_value).all(), "")
        check("V39_partial_excluded_sample_by_horizon", sample["partial_instruments_excluded_from_sample_gate_by_horizon"].map(bool_value).all(), "")
        check("V40_partial_excluded_positive_share_by_horizon", sample["partial_instruments_excluded_from_positive_instrument_share_by_horizon"].map(bool_value).all(), "")
        check("V41_concentration_formula_replayable", (paths.audit_dir / "r08_3_concentration_audit.csv").exists(), "")
        check("V42_H5_H10_diagnostic_pass_replayable", "diagnostic_horizon_positive" in diag.columns, "")
        check("V43_daily_cleanliness_failure_flag_exists", "daily_observation_spread_positive_but_cleanliness_failed" in gate.columns, "")
        check("V44_per_family_decision_replay_first_match", per_replay.groupby("family")["selected_rule_flag"].apply(lambda s: int(s.map(bool_value).sum())).eq(1).all(), "")
        check("V45_aggregate_decision_replay_first_match", int(agg_replay["selected_rule_flag"].map(bool_value).sum()) == 1, "")
        check("V46_aggregate_does_not_hide_family_decisions", set(per_final["family"]) == set(PRIMARY_FAMILIES), "")
        check("V47_authorized_strategy_requirement_false", per_final["authorized_strategy_requirement"].map(bool_value).eq(False).all() and aggregate_final["authorized_strategy_requirement"].map(bool_value).eq(False).all(), "")
        check("V48_final_decision_enum", aggregate_decision in FINAL_DECISIONS, aggregate_decision)
        check("V49_report_questions", all(f"{i}." in report for i in range(1, 39)), "")
        if (paths.cache_dir / "r08_3_oof_event_panel.parquet").exists():
            ev = pd.read_parquet(paths.cache_dir / "r08_3_oof_event_panel.parquet", columns=["fold_id", "instrument_fold_id"])
            check("V50_primary_evaluation_unseen_fold_only", ev["fold_id"].astype(int).eq(ev["instrument_fold_id"].astype(int)).all(), "")
    status = "passed" if not failures else "failed"
    audit = pd.DataFrame(checks)
    write_csv(audit, paths.audit_dir / "r08_3_validation_gate_audit.csv")
    payload = {
        ...
        "aggregate_final_decision": aggregate_decision,
        "failures": failures,
        "created_at": r01.now_iso(),
    }
    write_json(payload, paths.manifests_dir / "r08_3_validation.json")
    write_json({"created_at": r01.now_iso(), "artifacts": artifact_hashes(paths)}, paths.manifests_dir / "r08_3_artifact_hashes.json")
    return payload
