...
DEFAULT_CONFIG = EP5_DIR / "configs" / "r02_simple_rs20_continuation_v0.yaml"

REQUIREMENT_ID = "ep5_r02_simple_rs20_continuation_v0"
PLAN_ID = "ep5_e02_simple_rs20_continuation_v0"

PRIMARY_UNIT = "r02_simple_rs20_continuation_natural_exit_v0"
BASELINE_UNIT = "r02_weekly_liquid_universe_baseline_v0"
...
PRIORITY_RULES = [
    ...
    "rule_02_simple_continuation_supported",
    "rule_03_baseline_not_evaluable_validation_lead",
    "rule_04_beta_or_market_exposure_only",
    ...
    "rule_09_abs_positive_rel_negative_beta",
    "rule_10_horizon_specific_lead_only",
    "rule_11_sample_limited_primary_lead_only",
    "rule_12_no_simple_continuation_support",
]
...
CACHE_FILES = [
    "r02_daily_feature_panel.parquet",
    "r02_primary_event_panel.parquet",
    "r02_execution_event_panel.parquet",
    "r02_matched_comparator_panel.parquet",
    "r02_baseline_constituent_panel.parquet",
    "r02_right_tail_diagnostic_panel.parquet",
]

RUNNER_REPORTS = [
    "r02_artifact_authority.csv",
    "r02_input_data_audit.csv",
    "r02_provider_field_audit.csv",
    "r02_formula_input_coverage_audit.csv",
    "r02_formula_freeze_audit.csv",
    "r02_canonical_unit_registry.csv",
    "r02_rank_cross_section_audit.csv",
    "r02_event_generation_audit.csv",
    "r02_execution_block_audit.csv",
    "r02_denominator_audit.csv",
    "r02_comparator_scope_audit.csv",
    "r02_relative_denominator_audit.csv",
    "r02_comparator_fallback_quality_audit.csv",
    "r02_baseline_date_comparison.csv",
    "r02_baseline_lift_audit.csv",
    "r02_event_summary_by_unit_horizon_split.csv",
    "r02_event_summary_by_unit_horizon_year.csv",
    "r02_date_independence_audit.csv",
    "r02_sample_gate_audit.csv",
    "r02_concentration_gate_audit.csv",
    "r02_absolute_gate_audit.csv",
    "r02_relative_gate_audit.csv",
    "r02_robustness_confirmation_audit.csv",
    "r02_horizon_shape_audit.csv",
    "r02_multi_comparator_relative_audit.csv",
    "r02_regime_beta_decomposition.csv",
    "r02_rank_ret20_decomposition.csv",
    "r02_right_tail_readout.csv",
    "r02_right_tail_censoring_audit.csv",
    "r02_final_decision.csv",
    "r02_final_decision_inputs.csv",
]

RUNNER_MANIFESTS = [
    "r02_run_manifest.json",
    "r02_artifact_hashes.json",
]

VALIDATOR_REPORTS = [
    "r02_validation_gate_audit.csv",
    "r02_schema_validation_audit.csv",
    "r02_final_decision_replay_audit.csv",
    "r02_final_report.md",
]


class R02Error(RuntimeError):
    pass
...
def formula_spec(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "primary": {
            "ret20_window_trading_days": constants["ret20_window_trading_days"],
            "ret5_window_trading_days": constants["ret5_window_trading_days"],
            "ma20_window_trading_days": constants["ma20_window_trading_days"],
            ...
            "rank_pct_ret20_threshold": constants["rank_pct_ret20_threshold"],
            "ret5_sign_rule": constants["ret5_sign_rule"],
            "close_vs_ma20_rule": constants["close_vs_ma20_rule"],
            "weekly_observation_cadence": constants["weekly_observation_cadence"],
            "rank_cross_section_min_count": constants["rank_cross_section_min_count"],
        ...
        "baseline": {
            ...
            "baseline_scope": "weekly date-aligned full and nonselected liquid universe",
            "avg_money20_floor_cny": constants["avg_money20_floor_cny"],
            "min_nonselected_executable_constituents": 100,
            "baseline_status": [
                "comparable",
...
def build_input_audits(config: dict[str, Any], paths: r01.Paths) -> None:
    ...
    r01.write_csv(pd.DataFrame(rows), paths.reports_dir / "r02_artifact_authority.csv")
    ...
    r01.write_csv(audit, paths.reports_dir / "r02_input_data_audit.csv")
...
def ret_bucket(value: Any) -> str:
    ...
    if x <= 0.10:
        return "ret20_le_010"
    if x <= 0.20:
        return "ret20_010_020"
    if x <= 0.40:
        return "ret20_020_040"
    return "ret20_gt_040"
...
def build_feature_panel(config: dict[str, Any], paths: r01.Paths) -> pd.DataFrame:
    ...
    feature["ret20"] = grouped["close"].transform(lambda s: s / s.shift(int(constants["ret20_window_trading_days"])) - 1.0)
    ...
    feature["rank_pct_ret20"] = np.nan
    feature["rank_cross_section_count"] = 0
    feature["rank_cross_section_status"] = ""

    floor = float(constants["avg_money20_floor_cny"])
    min_count = int(constants["rank_cross_section_min_count"])
    audit_rows = []
    ...
    for date, group in weekly.groupby("trade_date", sort=True):
        rank_mask = (
            ...
            & group["close"].notna()
            & np.isfinite(group["ret20"])
            & np.isfinite(group["ma20"])
            & np.isfinite(group["ret5"])
        )
        rank_count = int(rank_mask.sum())
        status = "rank_cross_section_ok" if rank_count >= min_count else "blocked_insufficient_rank_cross_section"
        rank_values = group.loc[rank_mask, "ret20"]
        if rank_count:
            ranks = rank_values.rank(method="average", ascending=True) / rank_count
            feature.loc[ranks.index, "rank_pct_ret20"] = ranks
        feature.loc[group.index, "rank_cross_section_count"] = rank_count
        feature.loc[group.index, "rank_cross_section_status"] = status
        eligible_mask = (
            rank_mask
            & (rank_count >= min_count)
            & (group["close"] > group["ma20"])
            & (group["ret5"] > 0)
            & ((feature.loc[group.index, "rank_pct_ret20"]) >= float(constants["rank_pct_ret20_threshold"]))
            & (group["money"] > 0)
            & (group["volume"] > 0)
        )
        audit_rows.append(
            {
                ...
                "rank_cross_section_count": rank_count,
                "eligible_signal_count": int(eligible_mask.sum()),
                "rank_cross_section_status": status,
            }
        )
    feature["rank_pct_ret20_bucket_at_D"] = feature["rank_pct_ret20"].map(rank_bucket)
    feature["ret20_value_bucket_at_D"] = feature["ret20"].map(ret_bucket)

    r01.write_parquet(feature, paths.cache_dir / "r02_daily_feature_panel.parquet")
    r01.write_csv(pd.DataFrame(audit_rows), paths.reports_dir / "r02_rank_cross_section_audit.csv")
    ...
    r01.write_csv(provider_audit, paths.reports_dir / "r02_provider_field_audit.csv")
    coverage_fields = ["ret20", "ret5", "ma20", "avg_money20_D", "rank_pct_ret20"]
    coverage = [
        {
            "feature_name": column,
            "row_count": len(feature),
    ...
    r01.write_csv(pd.DataFrame(coverage), paths.reports_dir / "r02_formula_input_coverage_audit.csv")
    return feature
...
def build_unit_registry(config: dict[str, Any], paths: r01.Paths) -> None:
        ...
        [
            {
                ...
                "unit_role": "primary simple continuation exposure unit",
                "final_decision_authority": "sole R02 continue/no-continue decision unit",
                "horizons": "H5,H10,H20",
            ...
            },
            {
                ...
                "unit_role": "audit-only date-aligned weekly liquid universe baseline",
    ...
    r01.write_csv(registry, paths.reports_dir / "r02_canonical_unit_registry.csv")
            ...
            rows.append(
                {
                    ...
                    "source_requirement_section": "R02 §7",
                    "formula_name": unit,
    ...
    r01.write_csv(pd.DataFrame(rows), paths.reports_dir / "r02_formula_freeze_audit.csv")
...
def build_primary_event_panel(config: dict[str, Any], paths: r01.Paths, feature: pd.DataFrame) -> pd.DataFrame:
    ...
    floor = float(constants["avg_money20_floor_cny"])
    threshold = float(constants["rank_pct_ret20_threshold"])
    gap = int(constants["episode_merge_gap_trading_days"])
    eligible = (
        feature["weekly_observation_date"]
        & feature["split"].isin(SPLITS)
        & feature["pit_universe_member"]
        & (feature["avg_money20_D"] >= floor)
        & (feature["close"] > feature["ma20"])
        & (feature["ret5"] > 0)
        & (feature["rank_pct_ret20"] >= threshold)
        & (feature["money"] > 0)
        & (feature["volume"] > 0)
        & feature["rank_cross_section_status"].eq("rank_cross_section_ok")
    )
                ...
                rows.append(
                    {
                        ...
                        "signal_date": date,
                        "episode_id": f"{instrument}_{date.date()}_RS20_{episode_no}",
                        "episode_start_signal_date": date,
                        "event_key": event_key,
                        "split": record.split,
                        "detector_id": "R02_RS20_CONTINUATION_V0",
                        ...
                        "source_requirement_section": "R02 §7.1",
                        ...
                        "rank_pct_ret20": record.rank_pct_ret20,
                        "rank_cross_section_count": record.rank_cross_section_count,
                        "rank_pct_ret20_bucket_at_D": record.rank_pct_ret20_bucket_at_D,
                        "ret20_value_bucket_at_D": record.ret20_value_bucket_at_D,
    ...
    r01.write_parquet(event, paths.cache_dir / "r02_primary_event_panel.parquet")
    ...
    r01.write_csv(aggregate, paths.reports_dir / "r02_event_generation_audit.csv")
    return event
    ...
    for ev in event.itertuples(index=False):
        for horizon in HORIZONS:
            ...
            for attr in [
                ...
                "market_state",
                "beta_bucket",
                "is_full_liquid_baseline",
                "is_nonselected_liquid_baseline",
                "selected_primary_on_date",
            ]:
                if hasattr(ev, attr):
...
def write_execution_audits(paths: r01.Paths, execution: pd.DataFrame) -> None:
    ...
    r01.write_csv(block, paths.reports_dir / "r02_execution_block_audit.csv")
    r01.write_csv(denom, paths.reports_dir / "r02_denominator_audit.csv")
...
def build_execution_panel(config: dict[str, Any], paths: r01.Paths, feature: pd.DataFrame, event: pd.DataFrame) -> pd.DataFrame:
    ...
    r01.write_parquet(execution, paths.cache_dir / "r02_execution_event_panel.parquet")
...
def build_baseline_constituent_panel(config: dict[str, Any], paths: r01.Paths, feature: pd.DataFrame, primary_event: pd.DataFrame) -> pd.DataFrame:
    ...
    if primary_event.empty:
        empty = pd.DataFrame()
        r01.write_parquet(empty, paths.cache_dir / "r02_baseline_constituent_panel.parquet")
        return empty
    signal_dates = sorted(pd.to_datetime(primary_event["signal_date"]).dt.normalize().unique())
    selected = {
        pd.Timestamp(date): set(group["instrument_id"].astype(str))
        for date, group in primary_event.groupby("signal_date")
    }
    rows = []
    base = feature.loc[
    ...
    for record in base.itertuples(index=False):
        date = pd.Timestamp(record.trade_date)
        selected_on_date = record.instrument_id in selected.get(date, set())
        rows.append(
            {
                "canonical_unit_id": BASELINE_UNIT,
                "unit_role": "audit-only date-aligned weekly liquid universe baseline",
                ...
                "split": record.split,
                "formula_hash": hashes[BASELINE_UNIT],
                "source_requirement_section": "R02 §7.2",
                "is_full_liquid_baseline": True,
                "is_nonselected_liquid_baseline": not selected_on_date,
                "selected_primary_on_date": selected_on_date,
                "ret20": record.ret20,
                "ret5": record.ret5,
                "ma20": record.ma20,
                "avg_money20_D": record.avg_money20_D,
    ...
    r01.write_parquet(baseline_execution, paths.cache_dir / "r02_baseline_constituent_panel.parquet")
...
def build_comparator_panel(config: dict[str, Any], paths: r01.Paths, feature: pd.DataFrame, execution: pd.DataFrame) -> pd.DataFrame:
    ...
    r01.write_parquet(comparator, paths.cache_dir / "r02_matched_comparator_panel.parquet")
...
def write_comparator_audits(paths: r01.Paths, comparator: pd.DataFrame) -> None:
    ...
    r01.write_csv(scope, paths.reports_dir / "r02_comparator_scope_audit.csv")
    r01.write_csv(denom, paths.reports_dir / "r02_relative_denominator_audit.csv")
    r01.write_csv(fallback, paths.reports_dir / "r02_comparator_fallback_quality_audit.csv")
...
def build_baseline_date_comparison(
    ...
    for horizon in HORIZON_LABELS:
        for date in generated_dates:
            ...
            full_complete = baseline_date.loc[
                baseline_date.get("is_full_liquid_baseline", pd.Series(False, index=baseline_date.index)).astype(bool)
                & baseline_date["execution_status"].eq("complete_executable")
            ] if not baseline_date.empty else pd.DataFrame()
            nonselected_complete = baseline_date.loc[
                baseline_date.get("is_nonselected_liquid_baseline", pd.Series(False, index=baseline_date.index)).astype(bool)
                & baseline_date["execution_status"].eq("complete_executable")
            ] if not baseline_date.empty else pd.DataFrame()
            primary_decision = len(primary_complete) > 0
            baseline_count = len(nonselected_complete)
            ...
            nonselected_mean = safe_mean(nonselected_complete["net_return"])
            full_mean = safe_mean(full_complete["net_return"])
            rows.append(
                {
                    "canonical_unit_id": PRIMARY_UNIT,
                    "horizon": horizon,
                    ...
                    "full_baseline_executable_constituent_count": len(full_complete),
                    "full_baseline_date_equal_weight_return": full_mean,
                    "baseline_executable_constituent_count": baseline_count,
                    "baseline_comparison_status": status,
                    "baseline_date_equal_weight_return": nonselected_mean,
                    "selection_lift_vs_baseline": primary_mean - nonselected_mean if finite(primary_mean) and finite(nonselected_mean) else np.nan,
                    "primary_date_loss": bool(finite(primary_mean) and primary_mean < 0),
                    "baseline_date_loss": bool(finite(nonselected_mean) and nonselected_mean < 0),
                }
            )
    comparison = pd.DataFrame(rows)
    r01.write_csv(comparison, paths.reports_dir / "r02_baseline_date_comparison.csv")
    return comparison
...
def compute_summary_rows(
    ...
    for horizon in HORIZON_LABELS:
        for split in SPLITS:
            ...
            sample_gate = complete_count >= 600 and safe_share(complete_count, signal_count) >= 0.95 and year_count == 2 and min_year >= 200 and decision_count >= 70 and min_year_decision >= 30
            if complete_count >= 600:
                sample_status = "sample_pass"
            elif complete_count >= 300:
            ...
            date_independence_gate = bool(
                split == "validation"
                and decision_count >= 70
                and min_year_decision >= 30
                and top1_obs <= 0.05
                and top5_obs <= 0.20
            ...
            baseline_evaluable = baseline_count >= 70 and min_year_baseline >= 30
            ...
            baseline_lift_gate = bool(
                baseline_evaluable
                and split == "validation"
                and selection_lift_mean > 0
                and bool((baseline_year_mean >= -0.0025).all()) if len(baseline_year_mean) else False
            )
            if baseline_lift_gate:
                baseline_lift_gate = bool(
                    sum(
                        [
            ...
            row = {
                ...
                "mean_ret20_at_D": safe_mean(complete["ret20"]),
                "mean_rank_pct_ret20_at_D": safe_mean(complete["rank_pct_ret20"]),
                ...
                "rank_cross_section_count_min": int(rank_for_events["rank_cross_section_count"].min()) if len(rank_for_events) else 0,
                "rank_cross_section_count_median": float(rank_for_events["rank_cross_section_count"].median()) if len(rank_for_events) else np.nan,
    ...
    for idx, row in summary.iterrows():
        if row["split"] == "validation":
            ...
            robust = r02_robustness_criteria(summary, year_summary, row["horizon"])
    ...
    r01.write_csv(year_summary, paths.reports_dir / "r02_event_summary_by_unit_horizon_year.csv")
    r01.write_csv(summary, paths.reports_dir / "r02_event_summary_by_unit_horizon_split.csv")
    write_gate_audits(paths, summary)
    write_decompositions(paths, joined)
    return summary, year_summary


def r02_robustness_criteria(summary: pd.DataFrame, year_summary: pd.DataFrame, horizon: str) -> dict[str, Any]:
    ...
    confirmed = bool(
        int(r["complete_event_count"]) >= 600
        and float(r["complete_event_share"]) >= 0.95
        and int(r["year_count"]) == 2
        and int(r["min_year_complete_event_count"]) >= 200
        and int(r["decision_observation_date_count"]) >= 70
        and int(r["min_year_decision_observation_date_count"]) >= 30
...
def write_gate_audits(paths: r01.Paths, summary: pd.DataFrame) -> None:
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "complete_event_count", "complete_event_share", "year_count", "min_year_complete_event_count", "decision_observation_date_count", "min_year_decision_observation_date_count", "sample_status", "sample_gate_pass"]], paths.reports_dir / "r02_sample_gate_audit.csv")
    ...
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "mean_net_return", "median_net_return", "p10_net_return", "loss_rate", "absolute_positive"]], paths.reports_dir / "r02_absolute_gate_audit.csv")
    ...
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "decision_observation_date_count", "min_year_decision_observation_date_count", "date_weighted_mean_net_return", "date_weighted_mean_matched_delta_return", "positive_observation_date_share_net", "positive_observation_date_share_matched_delta", "top1_observation_date_event_share", "top5_observation_date_event_share", "date_independence_gate"]], paths.reports_dir / "r02_date_independence_audit.csv")
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "baseline_comparable_observation_date_count", "min_year_baseline_comparable_observation_date_count", "baseline_lift_evaluable", "selection_lift_vs_baseline_mean", "selection_lift_vs_baseline_median", "selection_lift_vs_baseline_p10", "selection_lift_loss_rate_delta", "baseline_lift_gate"]], paths.reports_dir / "r02_baseline_lift_audit.csv")
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "robustness_confirmed", "robustness_baseline_lift_evaluable", "robustness_selection_lift_vs_baseline_mean"]], paths.reports_dir / "r02_robustness_confirmation_audit.csv")
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "horizon_pass", "baseline_lift_gate", "strongly_negative", "adjacent_horizon_shape_status"]], paths.reports_dir / "r02_horizon_shape_audit.csv")
    r01.write_csv(summary[["canonical_unit_id", "horizon", "split", "mean_matched_delta_return", "same_day_universe_delta_mean", "industry_only_delta_mean", "liquidity_only_delta_mean", "SH000300_delta_mean", "relative_positive", "multi_comparator_relative_stable"]], paths.reports_dir / "r02_multi_comparator_relative_audit.csv")
...
def write_decompositions(paths: r01.Paths, joined: pd.DataFrame) -> None:
    ...
    rank_ret = decomposition_rows(complete, ["rank_pct_ret20_bucket_at_D", "ret20_value_bucket_at_D"])
    r01.write_csv(regime_beta, paths.reports_dir / "r02_regime_beta_decomposition.csv")
    r01.write_csv(rank_ret, paths.reports_dir / "r02_rank_ret20_decomposition.csv")
...
def build_summaries(
    ...
    baseline_date = build_baseline_date_comparison(paths, primary_event, execution, baseline)
    rank_audit = pd.read_csv(paths.reports_dir / "r02_rank_cross_section_audit.csv", parse_dates=["weekly_observation_date"])
...
def build_right_tail(config: dict[str, Any], paths: r01.Paths, feature: pd.DataFrame, execution: pd.DataFrame) -> pd.DataFrame:
    ...
    r01.write_parquet(panel, paths.cache_dir / "r02_right_tail_diagnostic_panel.parquet")
    ...
    r01.write_csv(readout, paths.reports_dir / "r02_right_tail_readout.csv")
    r01.write_csv(censor, paths.reports_dir / "r02_right_tail_censoring_audit.csv")
    return panel
...
def allowed_next(decision: str) -> tuple[str, str]:
    mapping = {
        "r02_simple_continuation_supported_continue_research": (
            "R03 controlled discovery under strict low-dimensional search, or a separately specified post-entry diagnostic",
        ...
        "r02_beta_or_market_exposure_only_no_stock_selection_pass": (
            "7.1 vs 7.2 beta / regime / liquidity attribution",
        ...
        "r02_no_simple_continuation_support": (
            "pause EP5 7.1 short-horizon exposure timing or select a new exposure mainline",
            "grid expansion; ret window/rank/horizon tuning",
        ),
        "r02_blocked_data_or_execution_contract": (
...
def all_priority_matches(summary: pd.DataFrame, contract_ok: bool = True) -> list[dict[str, Any]]:
    ...
    rule_defs = [
        ("rule_01_blocked_data_or_execution_contract", not contract_ok, "r02_blocked_data_or_execution_contract", "Required data, split, execution, cost, or canonical unit authority is missing."),
        ("rule_02_simple_continuation_supported", stock_selection_supported(summary) and robustness_confirmed(summary) and adjacent_clean(summary), "r02_simple_continuation_supported_continue_research", "Primary RS20 unit passed H10, baseline lift, robustness, and adjacent horizon checks."),
        ("rule_03_baseline_not_evaluable_validation_lead", baseline_not_evaluable_validation_lead(summary), "r02_baseline_not_evaluable_validation_lead", "Primary validation passed but date-aligned baseline was not evaluable."),
        ("rule_04_beta_or_market_exposure_only", h10_validated_pass(summary) and h10 is not None and bool_value(h10["baseline_lift_evaluable"]) and not bool_value(h10["baseline_lift_gate"]), "r02_beta_or_market_exposure_only_no_stock_selection_pass", "Primary validation passed but did not stably beat same-week nonselected liquid baseline."),
        ("rule_05_unstable_validation_only_lead", stock_selection_supported(summary) and not robustness_confirmed(summary), "r02_unstable_validation_only_lead", "Validation stock-selection support did not confirm in robustness."),
        ("rule_06_unstable_horizon_shape", stock_selection_supported(summary) and robustness_confirmed(summary) and adjacent_status(summary) == "adjacent_horizon_strongly_negative", "r02_unstable_horizon_shape_no_search_allowed", "H10 support was contradicted by a strongly negative adjacent horizon."),
        ("rule_07_adjacent_horizon_not_evaluable", stock_selection_supported(summary) and robustness_confirmed(summary) and adjacent_status(summary) == "adjacent_horizon_not_evaluable", "r02_adjacent_horizon_not_evaluable_validation_lead", "H10 support had an adjacent horizon that was not evaluable."),
        ("rule_08_relative_edge_only", relative_only_audit_supported(summary), "r02_relative_edge_only_hedged_or_regime_audit_required", "Relative edge exists without absolute long-only deployability and is stable across comparators."),
        ("rule_09_abs_positive_rel_negative_beta", h10 is not None and bool_value(h10["sample_gate_pass"]) and bool_value(h10["concentration_gate_pass"]) and bool_value(h10["date_independence_gate"]) and bool_value(h10["absolute_positive"]) and not bool_value(h10["relative_positive"]), "r02_beta_or_market_exposure_only_no_stock_selection_pass", "Absolute return exists without matched residual edge."),
        ("rule_10_horizon_specific_lead_only", horizon_specific_lead(summary), "r02_horizon_specific_lead_only_no_search_allowed", "Only H5 or H20 passed while H10 did not."),
        ("rule_11_sample_limited_primary_lead_only", sample_limited_return_lead(summary) and h10 is not None and bool_value(h10["baseline_lift_gate"]), "r02_sample_limited_primary_lead_only", "Primary unit is a sample-limited lead only."),
        ("rule_12_no_simple_continuation_support", True, "r02_no_simple_continuation_support", "No R02 rule supplied simple continuation support."),
    ]
    rows = []
    selected_seen = False
...
def build_final_decision(paths: r01.Paths, summary: pd.DataFrame) -> pd.DataFrame:
    ...
    r01.write_csv(final, paths.reports_dir / "r02_final_decision.csv")
    r01.write_csv(inputs, paths.reports_dir / "r02_final_decision_inputs.csv")
    r01.write_csv(final, paths.reports_dir / "r02_final_decision_replay_audit.csv")
    return final
...
def write_final_report(paths: r01.Paths, validation: dict[str, Any] | None, summary: pd.DataFrame, final: pd.DataFrame) -> None:
    ...
    lines.extend(
        [
            ...
            "3. 年度方向一致性见 `r02_event_summary_by_unit_horizon_year.csv`。",
            ...
            "8. H5 / H20 只作为 shape audit，不能替代 H10。",
            "9. Big-winner / right-tail 只在 `r02_right_tail_readout.csv` 中作为 post-entry diagnostic。",
            "10. 7.1 vs 7.2 的 baseline lift 已由 `r02_baseline_date_comparison.csv` 和 `r02_baseline_lift_audit.csv` 提供 authority。",
            "11. relative-only 必须通过 `multi_comparator_relative_stable`，不能只靠 SH000300。",
            f"12. 下一步允许 `{final_row.get('allowed_next_requirement', '')}`；禁止 `{final_row.get('blocked_next_requirements', '')}`。",
        ]
    )
    ...
    (paths.reports_dir / "r02_final_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_artifact_hashes(paths: r01.Paths) -> None:
    ...
    for name in ["r02_run_manifest.json", "r02_validation.json"]:
    ...
    r01.write_json({"created_at": r01.now_iso(), "artifacts": rows}, paths.manifests_dir / "r02_artifact_hashes.json")
...
def run_pipeline(config_path: str | Path = DEFAULT_CONFIG) -> None:
    ...
    r01.write_json(
        ...
        paths.manifests_dir / "r02_run_manifest.json",
    )
...
def validate_outputs(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    primary_event = pd.read_parquet(paths.cache_dir / "r02_primary_event_panel.parquet")
    execution = pd.read_parquet(paths.cache_dir / "r02_execution_event_panel.parquet")
    comparator = pd.read_parquet(paths.cache_dir / "r02_matched_comparator_panel.parquet")
    baseline = pd.read_parquet(paths.cache_dir / "r02_baseline_constituent_panel.parquet")
    right_tail = pd.read_parquet(paths.cache_dir / "r02_right_tail_diagnostic_panel.parquet")
    summary = pd.read_csv(paths.reports_dir / "r02_event_summary_by_unit_horizon_split.csv")
    baseline_date = pd.read_csv(paths.reports_dir / "r02_baseline_date_comparison.csv")
    final = pd.read_csv(paths.reports_dir / "r02_final_decision.csv")
    final_inputs = pd.read_csv(paths.reports_dir / "r02_final_decision_inputs.csv")
    ...
    check("baseline_comparable_count_threshold", bool((comparable_rows["baseline_executable_constituent_count"] >= 100).all()) if not comparable_rows.empty else True, "")
    blocked_rows = baseline_date.loc[baseline_date["baseline_comparison_status"].eq("blocked_insufficient_baseline_constituents")]
    check("baseline_blocked_count_threshold", bool((blocked_rows["baseline_executable_constituent_count"] < 100).all()) if not blocked_rows.empty else True, "")
    ...
    required_summary_cols = {"sample_gate_pass", "date_independence_gate", "baseline_lift_gate", "baseline_lift_evaluable", "multi_comparator_relative_stable"}
    ...
    h10 = summary_row(summary, "H10")
    if final.iloc[0]["final_decision"] == "r02_simple_continuation_supported_continue_research" and h10 is not None:
    ...
    if h10 is not None and h10_validated_pass(summary) and not bool_value(h10["baseline_lift_evaluable"]):
        check("baseline_not_evaluable_not_beta", final.iloc[0]["final_decision"] == "r02_baseline_not_evaluable_validation_lead", str(final.iloc[0]["final_decision"]))
    if final.iloc[0]["final_decision"] == "r02_relative_edge_only_hedged_or_regime_audit_required" and h10 is not None:
    ...
    year_summary = pd.read_csv(paths.reports_dir / "r02_event_summary_by_unit_horizon_year.csv")
...
def write_validation(
    ...
    r01.write_csv(gate, paths.reports_dir / "r02_validation_gate_audit.csv")
    r01.write_csv(gate[["check_name", "status", "detail"]], paths.reports_dir / "r02_schema_validation_audit.csv")
    replay_df = pd.DataFrame([replay or {"final_decision": final_decision, "priority_rule_id": ""}])
    r01.write_csv(replay_df, paths.reports_dir / "r02_final_decision_replay_audit.csv")
    payload = {
    ...
    r01.write_json(payload, paths.manifests_dir / "r02_validation.json")
