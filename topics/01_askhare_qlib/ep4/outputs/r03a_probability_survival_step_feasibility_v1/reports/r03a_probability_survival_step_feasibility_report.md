...
本报告的结论口径需要修正：R03a 不能证明 survival-step 对 `big winner` 有稳定增量，也不能把 `P_good / P_bad` 当成 `P(big winner | signal)` 的替代指标。

本实验的最终状态仍是：`blocked_no_stable_candidate_bucket`。但这个 blocked 的解释不是“survival-step 方向已经成立，只是 posterior bucket 不够稳定”。更准确的解释是：当前实验使用的是 R02 path label 里的 `good_path / bad_path / neutral_path`，而不是 canonical big winner label。`baseline_3_probe_survival_step_up` 的 `P_good` 提升主要来自 T+10 survival mask 过滤掉提前触发 -5% / bad_path 的 episode，使分母减少；它没有证明更多 episode 成为 big winner。

关键判断：

- `P_good / P_bad` 是 path-quality label posterior，不是 big-winner posterior。
- `baseline_3_probe_survival_step_up` 的 `P_good` 提升来自 denominator shrink：train / validation / robustness 的 `good_count` 没有增加，减少的是 `bad_count` 和部分 censored episode。
- T+10 survival 本身包含“没有提前跌破 -5%”的条件，因此它和 `bad_path` 定义不是独立关系；用它来证明 `P_bad` 下降有机械重合。
- 进一步加 probability gate 后看到的高 edge 小 bucket，也仍然是在 path-label 口径下，不是 big-winner edge。
- `fresh evidence`、`same-day bundle`、`context bucket` 在 R03a 中只允许描述性分析，不允许作为 gate。
- R03a 的研究价值应改写为：暴露了 path-label posterior 与 big-winner posterior 混用的风险，并否定了当前版本的 probability-bucket 冻结条件。

这里的 `denominator` 可以先按最基础的“样本数”理解：它是在当前统计口径下，真正进入这次概率计算的 episode 个数。比如某个 bucket 的 denominator 是 57，意思是这个 bucket 里只有 57 个可验证 episode 被用来计算 `P_good` / `P_bad`。`P_good=0.684` 不是 684 个好样本，而是 57 个 episode 里约 68.4% 被标成 good path。分母越小，百分比越容易被少数样本拉高或拉低，所以不能只看 `P_good` 高，还要看 denominator 是否足够大。

## 实验边界

R03a 是 probability-only feasibility audit，不是 EV_R sizing 实验。报告中的 R 标签只作为固定 exposure schedule label，不代表已经验证的仓位规模。

更重要的边界是：本报告里的 `P_good`、`P_bad` 不等于 big-winner precision。它们来自 episode 之后 120D 路径上的 `good_path` / `bad_path` 标签。`good_path` 可以代表较好的 continuation path，但不要求成为 canonical R01/R02 big winner；因此不能用 `P_good` 的提升直接替代 big-winner capture、big-winner precision 或 production entry edge。
...
全局 survival lift 只能说明 path-label posterior 发生变化，不能直接解释为 big-winner edge。
...
- survival 时间越长，episode 池越小，path-label 口径下的 `P_bad` 越低。
- T+10 的 `P_good=0.596` 不能解释为 big-winner probability，也不能解释为独立 alpha。
- T+10 的 `P_bad=0.353` 相对 T0 下降 26.0 个百分点，但这与 survival 条件本身排除了早期 -5% failure 有直接关系。
- 因此这更像是路径失败过滤后的分母重构，而不是一个独立发现的 posterior edge。
...
- 这些 lift 不应再表述为 survival-step 的独立正向证明。T+10 survival 的定义本身要求 episode 在 checkpoint 前没有触发 -5% low / close failure，而 `bad_path` 定义也包含 early failure / first -5% / max loss before +10，因此 `P_bad` 下降有明显的标签重合。
- global 和 multi_family_bundle 的 denominator 较大，只说明这个分母效应在大样本中稳定出现；它仍然不是 big-winner precision。
- price_trend、pullback_drawdown、momentum_rps 的 `P_good` 更高，也不能直接升格为 gate，因为它们仍是 path-label 口径且样本较小。
...
进一步拆开 count 后，Baseline 3 的 `P_good` 提升主要来自分母减少，而不是 good episode 增加：

| split | T0 good | baseline_3 good | good change | T0 bad | baseline_3 bad | bad change | denominator change |
|---|---:|---:|---:|---:|---:|---:|---:|
| train | 1,460 | 1,460 | 0 | 2,688 | 862 | -1,826 | -1,826 |
| validation | 729 | 729 | 0 | 1,406 | 447 | -959 | -959 |
| robustness | 879 | 879 | 0 | 1,183 | 508 | -675 | -675 |

研究含义：

- Baseline 3 在三个 split 中的 `P_good` 提升不是因为识别出更多 good path，而是因为 T+10 survival filter 删除了大量 bad path。
- `early failure` 变成 0 是 survival checkpoint 的定义结果，不是独立验证得到的交易优势。
- `max_gain_120d_p75` 没有明显下降，说明这个过滤没有完全丢掉 path-label upside；但它仍不能推出 big-winner capture 保持充分。
- 因此，原先“survival-step 作为观察结构通过方向性检验”的表述过强，应改为：**survival-step 是一个与 bad_path 定义高度重合的路径过滤器，当前实验不能证明它对 big winner posterior 有独立增量。**
...
这些行的表面信号很强，但 train OOF denominator 只有 57、63、143，不能满足冻结候选所需的 denominator/stability 约束。更基础地说，`denominator=57` 就是“只在 57 个历史 episode 上算出来的比例”：`P_good=0.684` 大约对应 39 个 good episode，`P_bad=0.298` 大约对应 17 个 bad episode。这个样本量太小，少几个 episode 的归类变化就会明显改变比例，所以它更可能是 bucket selection noise，不能冻结成规则。
...
这说明 blocked 不是由实现违约造成。但需要补充：本报告原先把 path-label posterior 的改善解释成 survival-step 的正向证据，这个解释口径过强。当前结果只能支持“path-label filter 之后的样本更少且 bad_path 更少”，不能支持“big-winner posterior 已提升”。
...
### 1. `P_good / P_bad` 不能替代 big-winner label

T+10 survival 把全局 episode 从 `P_good=0.356 / P_bad=0.613` 移动到 `P_good=0.596 / P_bad=0.353`。但这里的 `P_good / P_bad` 是 path label posterior，不是 big-winner posterior。它不能回答 `P(big winner | signal)`，也不能和 845 个 canonical R01/R02 big winner reference events 直接对齐。

更准确地说，这个变化说明 T+10 survival filter 删除了大量路径较差的 episode，尤其是提前触发 -5% 的 bad_path；它没有证明被保留下来的 episode 更可能成为 canonical big winner。
...
### 3. Baseline_3 的改善主要是分母效应

Baseline_3 的 `good_count` 在 train / validation / robustness 中与 T0 完全相同，`P_good` 上升来自 bad_path denominator 被大量过滤。大 denominator candidate 接近 baseline_3 时，edge 变成负值或不足，说明 posterior bucket 没有在这个过滤器之上给出稳定增量。

如果继续沿 probability bucket 方向推进，必须先换成或补充真正的 big-winner / executable-return label，而不是继续在 path-label posterior 上调 threshold。
...
### 不应把 survival-step 直接提升为 R03b 默认正向前提

R03a 的数据支持把 survival checkpoint 作为一个风险过滤诊断继续观察，但不支持把它当成已验证的 big-winner edge。后续实验如果继续使用 baseline_3，必须明确它是“提前 -5% 失败过滤后的 path-label baseline”，不是 stronger big-winner baseline。
...
- R03a 输出和约束校验通过，最终状态是 `blocked_no_stable_candidate_bucket`。
- `baseline_3` 在 path-label 口径下显著降低 bad_path denominator。
- 当前 probability posterior bucket 不能冻结。

本实验需要撤回或降级的结论：

- 不应说 survival-step 已经是有效的 big-winner 观察结构。
- 不应说 baseline_3 已经显著优于 T0 entry/probe-only，除非限定为 path-label denominator 过滤后的描述性结果。
- 不应把 `P_good / P_bad` 当作 big-winner precision。

本实验不支持的结论：

- 不支持冻结 probability posterior bucket。
- 不支持证明 `P(big winner | survival-step)` 提升。
...
1. 如果目标是 big-winner entry edge，先补一版 canonical big-winner posterior / executable-return posterior，不要继续用 `P_good / P_bad` 代替。
2. 如果目标是仓位与 risk budget，先实现 EV_R/action-time denominator 后再做 R03b。
