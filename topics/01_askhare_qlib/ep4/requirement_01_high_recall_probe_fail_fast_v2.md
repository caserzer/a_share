# EP4 Requirement 01 V2：Money-RPS + Boll/High10 Seed + Probe + Fail-Fast

> Requirement id: `ep4_r01_high_recall_probe_fail_fast_v2_money_rps5_boll20_high10_seed`
> Status: implementation-ready requirement
> Scope: EP4-R01 V2，只把 candidate seed 固定为 `money_ratio20_gt_1_0 AND money_ratio5_gt_2_0 AND rps5_gt_50 AND boll20_pct_b_gt_1_0 AND close_near_high10_gt_0`，重新验证小仓位观察权和 deterministic fail-fast 成本控制
...
inherited from R01 unless explicitly overridden below. V2 explicitly overrides
the candidate seed, adds the T+10 entry-price fail-fast trigger, and demotes
matched-random baselines to audit-only diagnostics.
```
...
```text
ep4/configs/r01_high_recall_probe_fail_fast_v2.yaml
```

Minimum required config:

```yaml
phase: ep4_r01_high_recall_probe_fail_fast_v2_money_rps5_boll20_high10_seed
requirement_path: ep4/requirement_01_high_recall_probe_fail_fast_v2.md
output_root: ep4/outputs/r01_high_recall_probe_fail_fast_v2_money_rps5_boll20_high10_seed
...
seed_recall:
  primary_recall_lookback_days: 60
  primary_recall_forward_days: 0
  primary_recall_window_id: primary_-60_0

seed_rules:
  baseline_seed_id: ep2_launch_detector_v0_bridge
  candidate_seed_id: ep4_money_rps5_boll20_high10_seed_v0
  candidate_seed_formula_id: money_ratio20_gt_1_0_and_money_ratio5_gt_2_0_and_rps5_gt_50_and_boll20_pct_b_gt_1_0_and_close_near_high10_gt_0
  candidate_seed_components_v2:
    money_activity:
      money_ratio20:
    ...
    structural_reference_fields:
      ...
      breakout_reference_rule: prior_10_trading_day_close_high
...
seed_density_caps:
  # V2 lowers the inherited R01 seed-day cap by 10%:
...
matched_controls:
  ...
  bucket_freeze_output: ep4/outputs/r01_high_recall_probe_fail_fast_v2_money_rps5_boll20_high10_seed/reports/r01_matched_control_bucket_freeze.csv
...
### 7.2 Candidate Seed：Money-RPS5 + Boll/High10 Seed

```text
seed_family_id = ep4_money_rps5_boll20_high10_seed_v0
```

V2 freezes the candidate seed as:
...
rps5_T          = percentile_rank(close_T / close_T-5 - 1)
                  within same-date PIT executable universe
...
The seed uses the prior 10-trading-day close high as the structural breakout
reference. For V2:
...
implementation may use the explicitly defined 10-day close-high reference, but
must not introduce any additional structural reference not listed above.

R01 V2 的 candidate seed 不要求是 EP2 detector 的 strict superset，但必须在方向上比 EP2 更偏高召回：

```text
新增 seed coverage 的成本必须单独报告；
relative strength is an explicitly frozen hard condition in V2, not a learned
or validation-selected threshold.
```

如果实现后发现 V2 candidate seed 的 seed-day count 或 episode count 低于 EP2 detector，R01 V2 不能以 high-recall path 通过；只能作为 negative diagnostic 报告。
...
failed_seed_primary =
  ...
  reference event under the primary seed recall window [-60, 0],
...
baseline_failed_seed_primary =
  ...
  [-60, 0], and primary_metric_eligible_baseline_event = true.
```
...
cannot be used as positive supporting evidence. In V2, this status does not
...
no_fail_fast_captured_reference_count =
  ...
  under the primary recall window [-60, 0] in same_seed_no_fail_fast_hold_h20
...
V2 lowers the 1-day matched-delay no-harm threshold by 10% from `-0.005` to
...
added_capture_vs_ep2_count =
  count(unique primary reference events captured by candidate seed
        under primary [-60, 0] recall window)
  - count(unique primary reference events captured by both candidate seed
          and EP2 bridge under primary [-60, 0] recall window)
...
| §12.4 matched baseline no-harm | required for matched-delay; matched-random report_only | required for matched-delay; matched-random report_only | matched-delay p05 / reliability required; matched-random report_only | fail if required matched-delay no-harm fails |
...
```text
ep4/outputs/r01_high_recall_probe_fail_fast_v2_money_rps5_boll20_high10_seed/
  cache/
...
| `candidate_captured_reference_count` | unique primary references captured by candidate under primary [-60, 0] window |
| `ep2_captured_reference_count` | unique primary references captured by EP2 bridge under primary [-60, 0] window |
| `overlap_captured_reference_count` | unique primary references captured by both under primary [-60, 0] window |
| `added_capture_vs_ep2_count` | candidate-only captured primary references under primary [-60, 0] window |
| `lost_capture_vs_ep2_count` | EP2-only captured primary references under primary [-60, 0] window |
