...
| episode_geometry_panel | required for primary episode-local features | step_id or stable join key, episode_cluster_id, step_start_pos, cluster_start_pos, cluster_end_pos, derived episode_low_pos_t0, derived episode_high_pos_t0, optional reclaim_pos_t0 | M1/M5 |
...
| M5 | episode position and maturity | high_medium | Report identifies missing episode-internal position; should provide structure orthogonal to single participation features |
| M2 | supply and pressure dynamics | medium | F2 is the only effective current source, but current F2 is mostly level/z-score; dynamic pressure morphology may add marginal information |
...
relative position within episode lifecycle
```
...
```text
fit train-only linear residualization:
...
For M2 money-flow in/out proxy candidates, an additional train-only incremental
residualization must be reported:
...
The M2 feature may be recommended only if the F2-extended residual still passes
the train-prior support rule. This prevents a new money-flow proxy from merely
renaming the existing participation/money z-score signal.
...
Only `train_priority_prior` rows may set `orthogonal_payoff_candidate = true`.
...
4. candidate_lineage_gate fail -> 18D_feature_representation_contract_blocked
5. pit_t0_availability_gate fail with no primary candidates -> 18D_no_pit_valid_candidate_features_found
6. pit_t0_availability_gate fail with delayed-only candidates -> 18D_candidate_features_delayed_or_leaky
7. orthogonal_payoff_information_gate fail -> 18D_no_orthogonal_payoff_information_found
8. feature_family_prioritization_gate fail -> 18D_representation_gap_diagnostic_only
9. search_accounting_gate fail -> 18D_search_accounting_blocked
10. all hard gates pass -> 18D_feature_representation_refresh_supported
```
...
6. Lineage/PIT/t0 audit summary.
7. Orthogonal payoff information summary after volatility/participation residualization.
8. Path entropy and repair path efficiency diagnostics, including window and binning definitions.
9. Money-flow inflow/outflow proxy diagnostics, including signed-flow proxy rule and F2-extended residualization.
10. Recommended feature families for refresh.
11. Families deferred or appendix-only, with reasons.
