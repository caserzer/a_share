...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run 08 risk-on / transition recall exploration."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        default="full",
        help="Validate config or run the experiment.",
    )
    parser.add_argument(
        "--max-instruments",
        type=int,
        default=None,
        help="Debug/draft instrument cap. Full publishable runs must omit this.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "event_instance_count" in result:
        ...
        print(f"selected_canonical_events={result['selected_canonical_event_count']}")
