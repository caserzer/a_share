...
authorization_file = references/21c_full/execution_authorization.json
scope_restart_decision_file = references/21c_full/scope_restart_decision.json
canonical_output_root = outputs/21C_full_reaka_pit_proxy_replication_v1
preauthorization_audit_root = outputs/21C_full_reaka_pit_proxy_replication_v1_preauthorization_audits
```
...
例外只限当前未授权 draft：在不存在 execution authorization、canonical root 或 preauthorization audit 的前提下，可原地修订 `v1`；一旦任一
...
相邻 pin exact-match。Runner 不得生成或修改 authorization、scope restart 或 upstream erratum 文件。

### 2.2 合格 corrected 21B successor

Config 必须复制 authorization 中的 exact pins。合格 successor 必须同时满足：
...
10. 21B config、decision、manifest 均显式记录 `upstream_21b_contract_erratum_gate=pass` 或等价的 corrected-version gate。
...
runtime_counter_evidence_sha256,post_cutoff_value_token_materialization_count,
...
`status=corrected_rerun_sealed`。`runtime_counter_evidence_path` 必须位于同一 corrected root、被 output manifest/hash closure 覆盖，且其 CSV
schema exact 为：

```text
process_id,stage,access_scope,operation,path_class,value_token_requested,value_decoded,
decision_date_min,decision_date_max,event_count,source_log_sha256,status,reason
```

Canonical sort：`process_id,stage,access_scope,operation,path_class`；counter 必须由真实 wrapper/log 聚合，禁止由 config 常量或报告文字生成。
...
若 corrected 21B 使用其他 route、feature count/order/expression hash，本 v1 fail；不得动态适配到 158 或其他 feature count。
...
- 只能进入 shared target encoder，构造 `Z_teacher_shifted`；
- 不得进入 source encoder input、GateNet input、selector context、DDPM condition 或 inference signature；
...
2. gumbel_noise = -log(-log(clamp(U,2^-24,1-2^-24)))
...
`not_run|not_evaluable` 在上述 `fail(any ...)` 中均按 fail 处理；一旦较早 state 命中，后续 gate 保留 `not_run`，不得伪造 pass。
...
固定 lowercase `true|false`，float 使用 round-trip decimal，日期 `YYYY-MM-DD`，timestamp RFC3339 UTC。禁止 extra columns。所有
`status` 只允许 `pass|fail|not_run|not_evaluable`，`reason` 在 pass 时为空、其他状态非空。
...
approved_21b,approved_21a_lineage,paths,feature_route,splits,
...
upstream_hash_and_file_set_gate,
dependency_runtime_gate,input_panel_integrity_gate,teacher_materialization_gate,
train_validation_firewall_gate,historical_holdout_zero_access_gate,
...
P0 with authorization_schema_status in {missing,invalid}: AUTH_PIN_FIELDS = null
P0 with authorization_schema_status=pass: AUTH_PIN_FIELDS = exact authorization pins
P1..P5: AUTH_PIN_FIELDS = non-null exact authorization pins
...
Final manifest 中的 `scope_restart_decision_sha256,approved_21b_output_root,approved_21b_output_hashes_sha256,`
...
全部 failure evidence。Canonical root 下出现 `U` 外任何 file、directory entry、symlink 或 profile-forbidden path 均使 seal fail。
...
1. 缺失/错误/extra-key/non-human execution authorization、scope restart 或任一 pinned hash 在任何 value/checkpoint open 前 fail；P0
   missing/invalid authorization 的 resolved-config/decision/manifest canonical null bytes exact；
2. current uncorrected 21B_v4 fixture 因 contract erratum gate fail，且 model/value open count=0；
3. corrected 21B successor 的 requirement/runner/config/test、两个 errata、runtime counter evidence、decision/manifest/output hashes/file set/
   semantic payload/pre-holdout bundle 全部 exact-match 才通过；
...
10. teacher tensor尝试进入 source/gate/selector/DDPM condition/inference signature fail；
...
14. soft Gumbel train、hard argmax inference/tie-smallest exact；
...
36. nine decision states first-match、P4 映射 finalize-integrity state、success-candidate/failure-bundle 两个 integrity gates互不覆盖、blocking
    reasons与authorization booleans exact；failure-bundle integrity fail禁止发布 profile；
...
- [ ] P0 missing/invalid authorization 的 resolved config、decision、manifest canonical null rules exact
...
- [ ] train-only teacher shift/index/value/hash闭合，validation teacher为0
- [ ] full R2 dual LSTM/gate/AKS/Koopman/DDPM/decoder/loss与21A一致
- [ ] R2 module registration/init/RNG draw/optimizer traversal、AdamW explicit flags/step、tau公式与contract hash exact
