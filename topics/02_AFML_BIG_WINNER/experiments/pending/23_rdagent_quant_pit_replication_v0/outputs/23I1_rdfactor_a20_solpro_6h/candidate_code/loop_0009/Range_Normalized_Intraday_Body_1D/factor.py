...
def calculate_range_normalized_intraday_body_1d():
    ...
    intraday_body = close_price - open_price
    daily_range = high_price - low_price
    factor = (intraday_body / daily_range).where(valid, np.nan)

    result = factor.to_frame(name="Range_Normalized_Intraday_Body_1D")
    result.index = result.index.set_names(["datetime", "instrument"])
    result.to_hdf(output_path, key="data", mode="w")


if __name__ == "__main__":
    calculate_range_normalized_intraday_body_1d()
